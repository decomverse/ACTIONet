...
using arma::Cube;
using arma::Col;
template<typename T> using py_arr = py::array_t<T, py::array::f_style | py::array::forcecast>; 
template <typename T> using py_arr_c = py::array_t<T, py::array::c_style | py::array::forcecast>;
using npint = npy_int;
...
template<typename T> using is_SpMat = aw::is_specialization_of<arma::SpMat, T>;
template<typename T> using is_Col = aw::is_specialization_of<arma::Col,T>;
template<typename T> using is_Row = aw::is_specialization_of<arma::Row,T>;
template<typename T> using is_Mat = aw::is_specialization_of<arma::Mat,T>;
template<typename T> using is_Cube = aw::is_specialization_of<arma::Cube,T>;
template<typename T> using is_Py_Arr = aw::is_specialization_of<py_arr, T>;
...
std::vector<npulong> extract_shape(const InfoType& info) {
    ...
    if(static_cast<npulong>(info.ndim) >  dim) {throw std::runtime_error("Incompatible buffer dimensions");}
...
py::buffer_info def_buffer(Mat<Scalar>&& m) {
    ...
    if(m.size() > 0) {
        ...
        return_buf = py::buffer_info(reinterpret_cast<void*>(m.memptr()), /* data */
                    ...
                    std::vector<ssize_t>{static_cast<ssize_t>(sizeof(Scalar)),
                        static_cast<ssize_t>(sizeof(Scalar)) * static_cast<ssize_t>(m.n_rows)} );
     ...
     if (m.size() >= 0) { 
        ...
        return_buf = py::buffer_info(reinterpret_cast<void*>(m.memptr()), /* data */
                    ...
                    std::vector<ssize_t>{static_cast<ssize_t>(sizeof(Scalar)) * static_cast<ssize_t>(m.n_rows) *
                        static_cast<ssize_t>(m.n_cols), static_cast<ssize_t>(sizeof(Scalar)),
                        static_cast<ssize_t>(sizeof(Scalar)) * static_cast<ssize_t>(m.n_rows)}); 
    } else {
...
/*                                                                                                                 
 * Converts something that may be an lvalue into an rvalue by copying if necessary.
 */  
template <typename T>
...
template <typename Scalar>
py::buffer_info def_buffer(const Mat<Scalar>& m) {return def_buffer<Scalar>(clone(m));}


template<typename Scalar>
py::buffer_info def_buffer(const Cube<Scalar>& m) {return def_buffer<Scalar>(clone(m));}

/* 
 * These two functions call make_py_arr function which takes an Rvalue, and so if they are given something 
 * else they copy it. 
 */
...
/*
 * This function converts a 3 dimensional numpy array into an armadillo cube. The tricky part is that the 
 * memory layout of the two arrays is different. Consequently, you need to adjust the various paramters. 
 */
template<typename ReturnType>
struct conv_to<ReturnType, typename is_Cube<ReturnType>::type> {
    ...
    static ReturnType from(py::array_t<Scalar, ForwardType>& b) {
        ...
        npulong length = std::accumulate(std::cbegin(shape), std::cend(shape), 1, std::multiplies<Scalar>()); 
...
struct conv_to<ReturnType, typename is_Row<ReturnType>::type> {
    ...
    static ReturnType from(py::array_t<Scalar, ForwardType> & b) {
        ...
        if (ForwardType != py::array::f_style && ForwardType != py::array::c_style) {
...
struct conv_to<ReturnType, typename is_Col<ReturnType>::type> {
    ...
    static ReturnType from(py::array_t<Scalar, ForwardType> & b) {
        ...
        if (b.writeable()) {
            ...
            arma::Col<Scalar> return_mat(shape[0]); auto ref_ptr = static_cast<Scalar*>(info.ptr);
            std::copy(ref_ptr, ref_ptr + shape[0], std::begin(return_mat)); return return_mat;
...
void claim_same_n_rows(const Args& ...args) { 
    ...
    if (!check_same_n_rows(args...)) {
        ...
        throw std::runtime_error("The number of rows in each of the "s + std::to_string(num_args) + 
...
void claim_same_n_cols(const Args& ...args) { 
    ...
    if (!check_same_n_cols(args...)) {
        ...
        std::size_t num_args = sizeof...(args);  
        throw std::runtime_error("The number of cols in each of the "s + std::to_string(num_args) + 
                                             " arrays is not the same."s + " The args are"s + stringify(args...)); 
    }
...
template<typename T1, typename T2, typename ...Args>
bool check_same_n_slices(const T1& first, const T2& second, const Args& ...args) { 
   
    return check_same_n_slices(first, second) && check_same_n_slices(second, args...); 
...
void claim_same_n_slices(const Args& ...args) { 
    ...
    if (!check_same_n_slices(args...)) {
        ...
        std::size_t num_args = sizeof...(args);  
        throw std::runtime_error("The number of slices in each of the "s + std::to_string(num_args) + 
                                             " arrays is not the same."s + " The args are"s + stringify(args...)); 
    }
...
void claim_same_size(const Args& ...args) { 
    ...
    if (!check_same_size(args...)) {
        ...
        std::size_t num_args = sizeof...(args);  
        throw std::runtime_error(std::string("All of the ") + std::to_string(num_args) + 
...
void claim_is_finite(const Args& ...args) { 
    ...
    if (!check_is_finite(args...)) {
        ...
        std::size_t num_args = sizeof...(args);  
        throw std::runtime_error(std::string("Not all of the ") + std::to_string(num_args) + 
...
/* 
 * Converts a vector to its square matrix as long as the vector has a Scalarber of elmeents that is a perfect square.
 */
template<typename ValueType>
Mat<ValueType> reshape_square(Col<ValueType>&& vec) { 

    npulong side_length = (npulong) std::sqrt(vec.n_elem);
    assert(side_length * side_length == vec.n_elem && "The vector does not have a perfect square Scalarber of "
                                                      "elements");
...
struct type_caster<Type, enable_if_t<aw::is_SpMat<Type>::value>> {
    ...
    bool load(handle src, bool) {
        ...
        arma::uvec row_vector =
            arma::conv_to<arma::uvec>::from(aw::conv_to<StorageIndexVector>::from(row_indices)); 
        ...
        value = arma::SpMat<Scalar>(row_vector, col_ptr_vector, values_vector, shape[0].cast<IndexType>(),
