...
get.pseudobulk.SE <- function(sce, batch_attr, ensemble = FALSE, bins = 20, assay = "counts", colData = NULL, pseudocount = 0, with_S = FALSE, with_E = FALSE, with_V = FALSE, BPPARAM = SerialParam()){
  ...
  nnz_feat_mean = sapply(counts.list, function(X) mean(Matrix::colSums(X > 0)))
...
variance.adjusted.DE.Limma <- function(se, design, slot_E = "counts", slot_V = NULL, W.mat = NULL, phenotype.name = NULL, min.covered.samples = 3){
  ...
  selected.vars = Matrix::colSums(d > 0) >= min.covered.samples

  selected.genes = setdiff(1:nrow(W.masked), which(Matrix::rowSums(apply(design.mat[, selected.vars], 2, function(x) {
    mm =  (x > 0)
    v = as.numeric( (Matrix::rowSums(W.masked[, mm]) == 0) > 0 )
    return(v)
  })) > 0 ))
