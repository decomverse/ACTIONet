...
mycircle <- function(coords, v = NULL, params) {
    ...
    mapply(coords[, 1], coords[, 2], vertex.color, vertex.frame.color, vertex.size, 
        vertex.frame.width, FUN = function(x, y, bg, fg, size, lwd) {
            graphics::symbols(x = x, y = y, bg = bg, fg = fg, lwd = lwd, circles = size, 
...
hashid_settings = function(salt, min_length = 0, alphabet = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890", 
    ...
    out = list(alphabet = alphabet_, salt = salt, guards = guards_, separator = separator_, 
...
gen.colors <- function(Pal, color.no, plot.cols) {
    ...
    new.colors = grDevices::rgb(grDevices::convertColor(color = arch.colors, from = "Lab", 
        to = "sRGB"))
...
doubleNorm <- function(Enrichment, log.transform = TRUE, min.threshold = 0) {
...
assess.label.local.enrichment <- function(P, Labels) {
    ...
    res = list(Label = updated.Labels, Confidence = updated.Labels.conf, Enrichment = logPval)
