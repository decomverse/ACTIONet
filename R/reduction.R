...
reduce.ace <- function(ace, reduced_dim = 50, max_iter = 10, assay_name = "logcounts", 
    reduction_slot = "ACTION", SVD_algorithm = 0, seed = 0) {
    ...
    if (is.matrix(S)) {
        ...
        reduction.out = reduce_kernel_full(S = S, reduced_dim = reduced_dim, iter = max_iter, 
            seed = seed, SVD_algorithm = SVD_algorithm)
        
    } else {
        
        reduction.out = reduce_kernel(S = S, reduced_dim = reduced_dim, iter = max_iter, 
            seed = seed, SVD_algorithm = SVD_algorithm)
    }
