...
#' @export
compute.cluster.feature.specificity <- function(ace, clusters, output_slot, assay_name = "logcounts") {
...
annotate.clusters.using.labels <- function(ace, clusters, labels) {
    ...
    logPvals = sapply(sort(unique(clusters)), function(i) {
        ...
        logPval = HGT_tail(population.size = pop.size, success.count = pos.size, 
            sample.size = sample.size, observed.success = success.size)
...
cluster.graph <- function(G, resolution_parameter = 0.5, initial.clustering = NULL, 
    ...
    if (!is.null(initial.clustering)) {
        ...
        if (is.signed) {
            clusters = as.numeric(signed_cluster(A = G, resolution_parameter = resolution_parameter, 
                initial_clusters_ = initial.clustering, seed = seed))
        } else {
            clusters = as.numeric(unsigned_cluster(A = G, resolution_parameter = resolution_parameter, 
                initial_clusters_ = initial.clustering, seed = seed))
        }
    } else {
        ...
        if (is.signed) {
            clusters = as.numeric(signed_cluster(A = G, resolution_parameter = resolution_parameter, 
                initial_clusters_ = NULL, seed = seed))
        } else {
            clusters = as.numeric(unsigned_cluster(A = G, resolution_parameter = resolution_parameter, 
                initial_clusters_ = NULL, seed = seed))
        }
...
HDBSCAN.clustering <- function(ace, minPoints = 30, minClusterSize = 30, archetype.slot = "H_unified") {
    ...
    out_list = run_HDBSCAN(X = X, minPoints = minPoints, minClusterSize = minClusterSize)
