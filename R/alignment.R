compute_pairwise_alignment <- function(reference_profile, query_profile, reduced_dim = 50, log_transform = FALSE, deflate = T, compute.Z = FALSE, sample.no = 1000, seed = 0) {
	set.seed(seed)
	...
	# Deflate?
	if(deflate == TRUE) {
		query_profile.red = reduce_kernel_full(query_profile, reduced_dim = reduced_dim, iter = 1000)
		reference_profile.red = reduce_kernel_full(reference_profile, reduced_dim = reduced_dim, iter = 1000)
		
		
	
		query_profile.deflated = query_profile + query_profile.red$A %*% t(query_profile.red$B)
		reference_profile.deflated = reference_profile + reference_profile.red$A %*% t(reference_profile.red$B)
	...
	reduced_dim = min(min(min(dim(query_profile)), min(dim(reference_profile)))-1, reduced_dim)
	
	# pc.out1 = prcomp(t(query_profile_centered), retx = TRUE, rank = reduced_dim)
	# pc.out2 = prcomp(t(reference_profile_centered), retx = TRUE, rank = reduced_dim)
	# V.query = pc.out1$rotation[, 1:reduced_dim]
...
compute_merged_ace_from_cell_alignment <-function(reference_ace, query_ace, cell_to_cell_alignment, reference_cell_labels = NULL, query_cell_labels = NULL, n_epochs = 500) {
	...
	proj.out = transform_layout(cell_to_cell_alignment, t(reference_ace$ACTIONet2D), t(reference_ace$ACTIONet3D), t(reference_ace$denovo_color), n_epochs = n_epochs)		
	
	combined.ace = ACTIONetExperiment(rowData = DataFrame(Id = c(rownames(reference_ace), rownames(query_ace))), colData = DataFrame(Id = c(colnames(reference_ace), colnames(query_ace)), dataset = c(rep("reference", ncol(reference_ace)), rep("query", ncol(query_ace)))))
...
plot_pairwise_alignment <- function(alignment, reference_labels = NULL, query_labels = NULL, normalize = FALSE) {
	W = alignment
	W[W < 0] = 0
...
compute_cell_alignments_from_archetype_alignments <- function(reference_ace, query_ace, alignment, alignment_threshold = 0.1, footprint_threshold = 0.1, reference_slot_name = "unified", query_slot_name = "unified") {
	...
	cell_to_cell_alignments = lapply(1:length(ii), function(j) {
		u = reference_footprint[, j]; u = u / max(u)
		v = query_footprint[, j]; v = v / max(v)
		...
		# df = data.frame(RNA.ace$Celltype[match.ii], ATAC.ace$Celltype[match.jj], match.w)
		
		M = sparseMatrix(i = match.ii, j = match.jj, x = match.w, dims = c(length(u), length(v)))	
		return(M)
	})
