...
#' @export
plot.individual.gene <- function(ace, labels, gene_name, features_use = NULL, assay_name = "logcounts", CPal = CPal20) {
...
visualize.markers <- function(ace, markers, features_use = NULL, assay_name = "logcounts", transparency.attr = NULL, trans.z.threshold = -0.5, trans.fact = 3, node.size = 1, CPal = "magma", alpha_val = 0.9, export_path = NA) {
  ...
  if (alpha_val > 0){
    expression_profile = impute.genes.using.ACTIONet(ace, marker_set, features_use = features_use, alpha_val = alpha_val)
  } else {
    expression_profile = assays(ace)[[assay_name]][match(marker_set, features_use), , drop = FALSE]
