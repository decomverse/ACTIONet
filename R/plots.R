...
#' @param label_attr Character vector of length NROW(data) or colname of 'colData(data)' containing cell labels of interest (clusters, celltypes, etc.).
#' @param color_attr Character vector of length NROW(data) or colname of 'colData(data)' containing hex colors to use for each point, or matrix/data.frame containing RGB values to pass to grDevices::rgb().
#' @param trans_attr Numeric vector of length NROW(data) or colname of 'colData(data)' used to compute point transparency. Smaller values are more transparent.
...
#' @param color_slot Name of entry in colMaps(data) containing RGB values for default point colors generated by 'layout_ACTIONet()'. Used only if no other coloring parameters are given or valid (default:'denovo_color').
