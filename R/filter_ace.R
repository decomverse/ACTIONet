...
#' @export
filter.ace <- function(ace, assay_name = "counts", min_cells_per_feat = NULL, min_feats_per_cell = NULL, 
    ...
    repeat {
        ...
        if (!is.null(min_umis_per_cell)) {
            umi_mask = fastColSums(SummarizedExperiment::assays(ace.fil)[[assay_name]]) >= 
                min_umis_per_cell
        ...
        if (!is.null(min_feats_per_cell)) {
            feature_mask = fastColSums(SummarizedExperiment::assays(ace.fil)[[assay_name]] > 
        ...
        if (!is.null(min_cells_per_feat)) {
            ...
            cell_count_mask = fastRowSums(SummarizedExperiment::assays(ace.fil)[[assay_name]] > 
                0) >= min_fc
...
filter.ace.by.attr <- function(ace, by, assay_name = "counts", min_cells_per_feat = NULL, 
    ...
    fil_names <- lapply(IDX, function(idx) {
        fil_list <- filter.ace(ace = ace[, idx], assay_name = assay_name, min_cells_per_feat = min_cells_per_feat, 
            min_umis_per_cell = min_umis_per_cell, max_umis_per_cell = max_umis_per_cell, 
            min_feats_per_cell = min_feats_per_cell, return_fil_ace = FALSE)
