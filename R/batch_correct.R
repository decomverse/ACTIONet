...
reduce.and.batch.correct.ace.fastMNN <- function(ace, batch_attr = NULL, assay_name = "logcounts", 
    ...
    set.seed(0)
    mnn.out <- batchelor::fastMNN(S, batch = mnn_batch, k = MNN_k, d = reduced_dim, 
        auto.merge = FALSE, merge.order = merge_order, cos.norm = FALSE, BPPARAM = BPPARAM)
...
reduce.and.batch.correct.ace.Harmony <- function(ace, batch_attr, reduced_dim = 50, 
    max_iter = 10, assay_name = "logcounts", norm_method = "none", reduction_slot = "ACTION", 
    seed = 0, SVD_algorithm = 0) {
    if (!require(harmony)) {
...
orthogonalize.ace.batch.simple <- function(ace, batch_attr, reduction_slot = "ACTION") {
    batch_attr = .get_ace_split_IDX(ace, attr = batch_attr, return_split_vec = TRUE) %>% 
        as.factor
...
#' @export
reduce.and.batch.orthogonalize.ace <- function(ace, design_mat, reduced_dim = 50, 
    max_iter = 10, assay_name = "logcounts", norm_method = "default", reduction_slot = "ACTION", 
    seed = 0, SVD_algorithm = 0) {
    ...
    ace = reduce.ace(ace, reduced_dim = reduced_dim, max_iter = max_iter, assay_name = assay_name, 
        norm_method = norm_method, reduction_slot = reduction_slot, seed = seed, 
