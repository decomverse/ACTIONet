...
reduce.and.batch.correct.ace.fastMNN <- function(ace, batch_attr, reduced_dim = 50, MNN_k = 20, return_V = FALSE, reduction_slot = "MNN", V_slot = NULL,  BPPARAM = SerialParam()) {
    ...
    SummarizedExperiment::assays(ace)[["logcounts"]] = as(SummarizedExperiment::assays(ace)[["logcounts"]], "sparseMatrix")
    ...
    set.seed(0)
    mnn.out <- do.call(batchelor::fastMNN, c(sce.list.norm, list(k = MNN_k, d = reduced_dim, auto.merge = FALSE, merge.order = merge_order, cos.norm = FALSE,
