...
normalize.ace <- function(ace, norm_method = "default", batch_attr = NULL, assay_name = "counts", 
    ...
    if (norm_method == "scran") {
        ...
        ace = normalize.scran(ace = ace, batch_attr = batch_attr, assay_name = assay_name, 
            BPPARAM = BPPARAM)
        
    } else if (norm_method == "multiBatchNorm") {
        
        ace = normalize.multiBatchNorm(ace = ace, batch_attr = batch_attr, assay_name = assay_name, 
            BPPARAM = BPPARAM)
        
    } else if (norm_method == "linnorm") {
        
        ace = normalize.Linnorm(ace = ace, assay_name = assay_name)
        
    } else {
        
        ace = normalize.default(ace = ace, assay_name = assay_name, log_scale = TRUE)
        norm_method = "default"
    }
