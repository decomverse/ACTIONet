...
#' @export
assess.geneset.enrichment.from.archetypes <- function(ace, associations, min.counts = 0, 
...
#' Geneset.enrichments = assess.geneset.enrichment.from.archetypes(ace, associations)
#' @export
assess.peakset.enrichment.from.archetypes <- function(ace, associations, min.counts = 0, 
    specificity.slot = "unified_feature_specificity") {
...
assess.geneset.enrichment.gProfiler <- function(genes, category = c("GO:BP", "REAC", 
    ...
    p = ggpubr::ggbarplot(sub.terms, x = "term_name", y = "logPval", sort.val = "asc", 
        orientation = "horiz", fill = col, xlab = "", ylab = "") + ggplot2::geom_hline(yintercept = -log10(0.05), 
        col = "gray", lty = 2)
...
compute.phi <- function(A, labels, s0, s1, s2) {
    ...
    E2 = 4 * n^3 * Q1 - 4 * n^3 * k * Q1 + n^3 * k^2 * Q1 - 2 * (2 * n^2 * Q2 - n^2 * 
        k * Q2) + 2 * n * Q3 - n^2 * Q22
    ...
    B1 = 4 * n^3 * Q1 - 4 * n^3 * k * Q1 + n^3 * k^2 * Q1 - (2 * n^2 * Q2 - n^2 * 
        k * Q2)
...
#' @export
get.top.marker.genes <- function(ace, clusters, top_genes = 10, most_specific = FALSE, 
    features_use = NULL, feat_subset = NULL, assay_name = "logcounts", return_type = c("data.frame", 
        "df", "list")) {
    ...
    ace = compute.cluster.feature.specificity(ace = ace, clusters = cluster_vec, 
        output_slot = "temp_slot", assay_name = assay_name)
    ...
    if (most_specific == TRUE) {
        W = select.top.k.features(feat_spec, top_features = top_genes, normalize = FALSE, 
    ...
    if (return_type == "list") 
        return(as.list(df)) else return(df)
}
