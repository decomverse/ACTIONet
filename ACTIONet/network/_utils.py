...
def construct_backbone(
    ...
    if footprint_key not in adata.obsm.keys():
        compute_network_diffusion(adata, archetypes_key=archetypes_key, alpha=footprint_alpha, n_threads=n_threads)

    archetype_footprint = adata.obsm[footprint_key]
    if scale:
        W = np.exp(
            (archetype_footprint - np.mean(archetype_footprint, axis=0)) / np.std(archetype_footprint, axis=0, ddof=1)
        )
    else:
