...
std::vector<double> hdbscanStar::hdbscanAlgorithm::calculateCoreDistances(
  ...
  for (int point = 0; point < length; point++) {
    std::vector<double> kNNDistances(
        numNeighbors); // Sorted nearest distances found so far
    ...
    for (int neighbor = 0; neighbor < length; neighbor++) {
      ...
      if (neighborIndex < numNeighbors) {
        for (int shiftIndex = numNeighbors - 1; shiftIndex > neighborIndex;
             shiftIndex--) {
...
undirectedGraph hdbscanStar::hdbscanAlgorithm::constructMst(
  ...
  undirectedGraph undirectedGraphObject(
      length, nearestMRDNeighbors, otherVertexIndices, nearestMRDDistances);
  return undirectedGraphObject;
}

void hdbscanStar::hdbscanAlgorithm::computeHierarchyAndClusterTree(
    undirectedGraph *mst, int minClusterSize,
    std::vector<hdbscanConstraint> constraints,
    std::vector<std::vector<int>> &hierarchy,
    std::vector<double> &pointNoiseLevels, std::vector<int> &pointLastClusters,
  ...
  // cluster cluster_object(1, NULL, std::numeric_limits<double>::quiet_NaN(),
  // mst->getNumVertices());
  clusters.push_back(new cluster(1, NULL,
                                 std::numeric_limits<double>::quiet_NaN(),
                                 mst->getNumVertices()));

  std::set<int> clusterOne;
  clusterOne.insert(1);
  calculateNumConstraintsSatisfied(clusterOne, clusters, constraints,
  ...
  while (currentEdgeIndex >= 0) {
    ...
    while (currentEdgeIndex >= 0 &&
      ...
      std::vector<int> &firstVertexEdgeList =
          mst->getEdgeListForVertex(firstVertex);
      ...
      if (secondVertexInFirstEdgeList !=
          mst->getEdgeListForVertex(firstVertex).end())
        mst->getEdgeListForVertex(firstVertex)
            .erase(secondVertexInFirstEdgeList);
      std::vector<int> &secondVertexEdgeList =
          mst->getEdgeListForVertex(secondVertex);
      std::vector<int>::iterator firstVertexInSecondEdgeList =
          std::find(secondVertexEdgeList.begin(), secondVertexEdgeList.end(),
                    firstVertex);
      if (firstVertexInSecondEdgeList !=
          mst->getEdgeListForVertex(secondVertex).end())
        mst->getEdgeListForVertex(secondVertex)
            .erase(firstVertexInSecondEdgeList);
    ...
    while (affectedClusterLabels.size()) {
      ...
      for (auto affectedIt = affectedVertices.begin();
           affectedIt != affectedVertices.end();) {
      ...
      while (examinedVertices.size()) {
        ...
        while (unexploredSubClusterPoints.size()) {
          ...
          std::vector<int> &vertexToExploreEdgeList =
              mst->getEdgeListForVertex(vertexToExplore);
          for (std::vector<int>::iterator it = vertexToExploreEdgeList.begin();
               it != vertexToExploreEdgeList.end();) {
            int neighbor = *it;
            anyEdges = true;
            if (std::find(constructingSubCluster.begin(),
          ...
          if (!incrementedChildCount &&
              constructingSubCluster.size() >= minClusterSize && anyEdges) {
        ...
        if (numChildClusters >= 2 &&
            constructingSubCluster.size() >= minClusterSize && anyEdges) {
          // Check this child cluster is not equal to the unexplored first child
          // cluster:
          int firstChildClusterMember = *prev(firstChildCluster.end());
          if (std::find(
                  constructingSubCluster.begin(), constructingSubCluster.end(),
      ...
      if (numChildClusters >= 2 &&
        ...
        cluster *newCluster =
            createNewCluster(firstChildCluster, currentClusterLabels,
    ...
    for (std::vector<cluster *>::iterator it = newClusters.begin();
         it != newClusters.end(); it++) {
    ...
    if (newClusterLabels.size())
      calculateNumConstraintsSatisfied(newClusterLabels, clusters, constraints,
                                       currentClusterLabels);
...
std::vector<int> hdbscanStar::hdbscanAlgorithm::findProminentClusters(
    std::vector<cluster *> &clusters, std::vector<std::vector<int>> &hierarchy,
    int numPoints) {
  ...
  // Store all the hierarchy positions at which to find the birth points for the
  // flat clustering:
  ...
  while (significantHierarchyPositions.size()) {
    std::map<int, std::vector<int>>::iterator entry =
        significantHierarchyPositions.begin();
...
std::vector<double> hdbscanStar::hdbscanAlgorithm::findMembershipScore(
    std::vector<int> clusterids, std::vector<double> coreDistances) {
  ...
  while (i < length) {
    if (prob[i] == std::numeric_limits<double>::max()) {
      ...
      while ((iter = std::find(iter, clusterids.end(), clusterno)) !=
             clusterids.end()) {
      ...
      double maxCoreDistance =
          *max_element(tempCoreDistances.begin(), tempCoreDistances.end());
      for (int j = 0; j < tempCoreDistances.size(); j++) {
        prob[indices[j]] =
            (maxCoreDistance - tempCoreDistances[j]) / maxCoreDistance;
      }
    }
...
bool hdbscanStar::hdbscanAlgorithm::propagateTree(
    std::vector<cluster *> &clusters) {
  ...
  while (clustersToExamine.size()) {
    std::map<int, cluster *>::iterator currentKeyValue =
        prev(clustersToExamine.end());
...
/// calling this method.
/// </summary>
/// <param name="clusters">A list of Clusters forming a cluster tree which has
/// already been propagated</param> <param name="pointNoiseLevels">A double[]
/// with the levels at which each point became noise</param> <param
/// name="pointLastClusters">An int[] with the last label each point had before
/// becoming noise</param> <param name="coreDistances">An array of core
/// distances for each data point</param> <returns>An List of OutlierScores,
/// sorted in descending order</returns>
std::vector<outlierScore> hdbscanStar::hdbscanAlgorithm::calculateOutlierScores(
    std::vector<cluster *> &clusters, std::vector<double> &pointNoiseLevels,
    std::vector<int> &pointLastClusters, std::vector<double> coreDistances) {
  ...
  for (int i = 0; i < numPoints; i++) {
    double epsilonMax =
        clusters[pointLastClusters[i]]->PropagatedLowestChildDeathLevel;
...
/// <param name="clusterLabels">An array of cluster labels, which will be
/// modified</param> <param name="parentCluster">The parent Cluster of the new
/// Cluster being created</param> <param name="clusterLabel">The label of the
/// new Cluster </param> <param name="edgeWeight">The edge weight at which to
/// remove the points from their previous Cluster</param> <returns>The new
/// Cluster, or null if the clusterId was 0</returns>
cluster *hdbscanStar::hdbscanAlgorithm::createNewCluster(
    std::set<int> &points, std::vector<int> &clusterLabels,
...
/// <param name="clusterLabels">An array of current cluster labels for
/// points</param>
void hdbscanStar::hdbscanAlgorithm::calculateNumConstraintsSatisfied(
    std::set<int> &newClusterLabels, std::vector<cluster *> &clusters,
    std::vector<hdbscanConstraint> &constraints,
  ...
  for (int label : newClusterLabels) {
    ...
    if (parent != NULL &&
        !(find(parents.begin(), parents.end(), *parent) != parents.end()))
  ...
  for (hdbscanConstraint constraint : constraints) {
    ...
    if (constraint.getConstraintType() == hdbscanConstraintType::mustLink &&
        labelA == labelB) {
      if (find(newClusterLabels.begin(), newClusterLabels.end(), labelA) !=
          newClusterLabels.end())
        clusters[labelA]->addConstraintsSatisfied(2);
    } else if (constraint.getConstraintType() ==
                   hdbscanConstraintType::cannotLink &&
               (labelA != labelB || labelA == 0)) {
      if (labelA != 0 && find(newClusterLabels.begin(), newClusterLabels.end(),
                              labelA) != newClusterLabels.end())
        clusters[labelA]->addConstraintsSatisfied(1);
      if (labelB != 0 && (find(newClusterLabels.begin(), newClusterLabels.end(),
                               labelA) != newClusterLabels.end()))
        clusters[labelB]->addConstraintsSatisfied(1);
      if (labelA == 0) {
        for (cluster parent : parents) {
          if (parent.virtualChildClusterConstraintsPoint(
