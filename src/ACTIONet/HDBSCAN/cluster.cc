...
cluster::cluster() { _id = ++counter; }

cluster::cluster(int label, cluster *parent, double birthLevel,
                 int numPoints) //:Label(label), Parent(parent),
                                //_birthLevel(birthLevel), _numPoints(numPoints)
{
  _id = ++counter;
  _deathLevel = 0;
...
void cluster::propagate() {
  if (Parent != NULL) {
    ...
    if (PropagatedLowestChildDeathLevel <
        Parent->PropagatedLowestChildDeathLevel)
    ...
    if (!HasChildren) {
      ...
      Parent->_propagatedNumConstraintsSatisfied +=
          _propagatedNumConstraintsSatisfied;
      Parent->_propagatedStability += _propagatedStability;
      Parent->PropagatedDescendants.insert(Parent->PropagatedDescendants.end(),
      ...
      if (Stability >= _propagatedStability) {
        ...
        Parent->_propagatedNumConstraintsSatisfied +=
            _propagatedNumConstraintsSatisfied;
        Parent->_propagatedStability += _propagatedStability;
        Parent->PropagatedDescendants.insert(
            Parent->PropagatedDescendants.end(), PropagatedDescendants.begin(),
            PropagatedDescendants.end());
      }
    }
  }
...
void cluster::releaseVirtualChildCluster() { _virtualChildCluster.clear(); }

int cluster::getClusterId() { return this->_id; }
