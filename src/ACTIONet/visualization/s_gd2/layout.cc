...
void sgd(double *X, vector<term> &terms, const vector<double> &etas,
         const int seed) {
...
vector<vector<int>> build_graph_unweighted(int n, int m, int *I, int *J) {
  ...
  for (int ij = 0; ij < m; ij++) {
    ...
    if (i != j &&
        undirected[j].find(i) == undirected[j].end()) // if edge not seen
    {
...
vector<term> bfs(int n, int m, int *I, int *J) {
  ...
  for (int source = 0; source < n - 1;
       source++) // no need to do final vertex because i<j
  {
    ...
    if (terms.size() != terms_size_goal) {
      throw std::invalid_argument(
          "graph is not strongly connected, or is not indexed from zero");
    }
  }
  return terms;
}

vector<vector<edge>> build_graph_weighted(int n, int m, int *I, int *J,
                                          double *V) {
  ...
  for (int ij = 0; ij < m; ij++) {
    ...
    if (i != j &&
        undirected[j].find(i) == undirected[j].end()) // if key not there
    {
...
vector<double> schedule_convergent(const vector<term> &terms, int t_max,
                                   double eps, int t_maxmax) {
  double w_min = terms[0].w, w_max = terms[0].w;
  for (unsigned i = 1; i < terms.size(); i++) {
    const double &w = terms[i].w;
    if (w < w_min)
...
void sgd_threshold(double *X, vector<term> &terms, const vector<double> &etas,
                   double delta, const int seed) {
  // seed random number generator
  // std::minstd_rand rng(seed);
  // srand(seed);
  rk_state rstate;
...
void sgd3D(double *X, vector<term> &terms, const vector<double> &etas,
           const int seed) {
  // seed random number generator
  // std::minstd_rand rng(seed);
  // srand(seed);
  ...
  for (unsigned i_eta = 0; i_eta < etas.size(); i_eta++) {
    ...
    for (unsigned i_term = 0; i_term < n_terms; i_term++) {
      ...
      double dx = X[i * 3] - X[j * 3], dy = X[i * 3 + 1] - X[j * 3 + 1],
             dz = X[i * 3 + 2] - X[j * 3 + 2];
...
void mds_direct(int n, int kd, double *X, double *d, double *w, int t_max,
  ...
  if (kd == 2)
    ...
    throw std::invalid_argument(
        "only 2 or 3 dimensional layouts are supported");
}
