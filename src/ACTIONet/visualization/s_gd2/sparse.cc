...
void sgd(double *X, vector<term_sparse> &terms, const vector<double> &etas,
         const int seed) {
...
vector<int> maxmin_random_sp_unweighted(const vector<vector<int>> &graph,
                                        int n_pivots, int p0, int seed) {
  int n = graph.size();
...
void maxmin_bfs_unweighted(const vector<vector<int>> &graph, const int p,
                           vector<int> &mins, vector<int> &argmins) {
  int n = graph.size();
  std::queue<int> q;
  vector<int> d(n, -1);
...
// is not actually a multi-source shortest path, because regions come for free
// with maxmin_random_sp
vector<term_sparse> MSSP_unweighted(const vector<vector<int>> &graph,
                                    const vector<int> &closest_pivots) {
  int n = graph.size();
        ...
        if (d[next] == -1) {
          ...
          if (i < p) {
            ...
            if (termsDict[i].find(p) == termsDict[i].end())
              termsDict[i].insert(
                  std::pair<int, term_sparse>(p, term_sparse(i, p, d[next])));
            ...
            if (termsDict[p].find(i) == termsDict[p].end())
              termsDict[p].insert(
                  std::pair<int, term_sparse>(i, term_sparse(p, i, d[next])));
    ...
    for (unsigned i_edge = 0; i_edge < graph[i].size(); i_edge++) {
      ...
      if (i < j) {
        ...
        if (termsDict[i].find(j) == termsDict[i].end())
          termsDict[i].insert(
              std::pair<int, term_sparse>(j, term_sparse(i, j, 1)));
        else
          // termsDict[i].at(j).d = 1;
          termsDict[i].find(j)->second.d = 1;

        // termsDict[i].at(j).w_ij = termsDict[i].at(j).w_ji = 1;
        termsDict[i].find(j)->second.w_ij = termsDict[i].find(j)->second.w_ji =
            1;
      }
    }
  }
...
vector<int> maxmin_random_sp_weighted(const vector<vector<edge>> &graph,
                                      int n_pivots, int p0, int seed) {
  int n = graph.size();
...
void maxmin_bfs_weighted(const vector<vector<edge>> &graph, const int p,
                         vector<double> &mins, vector<int> &argmins) {
  int n = graph.size();
  vector<bool> visited(n, false);
  vector<double> d(n,
                   std::numeric_limits<double>::max()); // init 'tentative'
...
// again, not a proper MSSP because we get regions for free with
// maxmin_random_sp
vector<term_sparse> MSSP_weighted(const vector<vector<edge>> &graph,
                                  const vector<int> &closest_pivots) {
  int n = graph.size();
  ...
  for (region = regions.begin(); region != regions.end(); ++region) {
    ...
    vector<bool> visited(n, false);
    vector<double> d(
        n, std::numeric_limits<double>::max()); // init 'tentative' distances to
                                                // infinity
    ...
    while (!pq.empty()) {
      ...
      if (!visited[current]) // ignore redundant elements in queue
      {
        ...
        if (i < p) {
          ...
          if (termsDict[i].find(p) == termsDict[i].end())
            termsDict[i].insert(
                std::pair<int, term_sparse>(p, term_sparse(i, p, d_pi)));
          ...
          if (termsDict[p].find(i) == termsDict[p].end())
            termsDict[p].insert(
                std::pair<int, term_sparse>(i, term_sparse(p, i, d_pi)));
  ...
  for (int i = 0; i < n; i++) {
    for (unsigned i_edge = 0; i_edge < graph[i].size(); i_edge++) {
      ...
      if (i < j) {
        ...
        if (termsDict[i].find(j) == termsDict[i].end())
          termsDict[i].insert(
              std::pair<int, term_sparse>(j, term_sparse(i, j, d_ij)));
        else
...
vector<double> schedule(const vector<term_sparse> &terms, int t_max,
                        double eps) {
