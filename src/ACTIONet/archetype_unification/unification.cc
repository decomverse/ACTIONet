...
unification_results unify_archetypes(sp_mat &G, mat &S_r, mat &C_stacked,
  ...
  stdout_printf(
      "\tParameters: alpha = %0.2f, outlier_threshold = %0.2f, sim_threshold = "
      "%0.2f\n",
  ...
  for(int j = 0; j < logC.n_cols; j++) {
          vec v = logC.col(j);
          uvec idx = find( (v != datum::nan) && (v != -datum::inf) && (v !=
  +datum::inf)); double mu = mean(v(idx)); double sigma = stddev(v(idx)); vec z
  = (v-mu)/sigma; uvec mask_idx = find( (v == datum::nan) || (v == -datum::inf)
  || (v == +datum::inf)); z(mask_idx).zeros(); Z.col(j) = z;
  }
...
*/
  ...
  */
  ...
  /*
                  ...
                  uvec selected_archetypes = find( (zz > outlier_z_threshold) &&
     (Sim.diag() != 0) );
  */
  Sim.transform(
      [sim_threshold](double val) { return (val < sim_threshold ? 0 : val); });
