...
namespace ACTIONet {
	...
	mat compute_network_diffusion_direct(sp_mat &G, sp_mat &X0, int thread_no = 4, double alpha = 0.85) {
		...
		printf("Creating A\n"); fflush(stdout);
		cholmod_triplet *T = cholmod_allocate_triplet(P.n_rows, P.n_cols, P.n_nonzero, 0, CHOLMOD_REAL, &c);
		T->nnz = P.n_nonzero;
		...
		T = cholmod_allocate_triplet(X0.n_rows, X0.n_cols, X0.n_nonzero, 0, CHOLMOD_REAL, &c);
		T->nnz = X0.n_nonzero;
		Ti = static_cast<int *>(T->i);
		Tj = static_cast<int *>(T->j);
		Tx = static_cast<double *>(T->x);
		...
		mat PR = mat(sp_mat(locations, (1-alpha)*vec(Tx, T->nnz), X0.n_rows, X0.n_cols));
