...
namespace ACTIONet {	
    ...
    sp_mat compute_sparse_network_diffusion(sp_mat& A, sp_mat& U, double alpha = 0.85, double rho = 1e-4, double epsilon = 0.001, int max_iter = 20) {
        ...
        sp_mat Q = get_Q(A, ds_inv, alpha); // .5 * ((1 + alpha)*I - (1 - alpha)*(D^(-1/2) * A * D^(-1/2)));
