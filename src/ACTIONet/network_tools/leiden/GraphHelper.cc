...
Graph::Graph(igraph_t *graph, vector<double> const &edge_weights,
  ...
  if (edge_weights.size() != this->ecount())
    throw Exception("Edge weights vector inconsistent length with the edge "
                    "count of the graph.");
  this->_edge_weights = edge_weights;
  this->_is_weighted = true;

  if (node_sizes.size() != this->vcount())
    throw Exception("Node size vector inconsistent length with the vertex "
                    "count of the graph.");
  this->_node_sizes = node_sizes;

  if (node_self_weights.size() != this->vcount())
    throw Exception("Node self weights vector inconsistent length with the "
...
size_t Graph::possible_edges() { return this->possible_edges(this->vcount()); }
...
void Graph::init_admin() {
  ...
  // Calculcate strength
  igraph_strength(this->_graph, &res, igraph_vss_all(), IGRAPH_IN, true,
                  &weights);
  ...
  igraph_vector_init_copy(&weights, &this->_edge_weights[0], this->ecount());
  // Calculcate strength
  igraph_strength(this->_graph, &res, igraph_vss_all(), IGRAPH_OUT, true,
                  &weights);
  igraph_vector_destroy(&weights);
...
// Return the neighbour at that index
      ...
      // Return the neighbour at that index
      rand_neigh = VECTOR(
          this->_graph->from)[(size_t)VECTOR(this->_graph->ii)[rand_neigh_idx]];
    }
  } else {
...
Graph *Graph::collapse_graph(MutableVertexPartition *partition) {
  ...
  vector<map<size_t, double>> collapsed_edge_weights(
  ...
  for (vector<map<size_t, double>>::iterator itr =
           collapsed_edge_weights.begin();
  ...
  igraph_vector_init(&edges,
                     2 * m_collapsed); // Vector or edges with edges (edge[0],
                                       // edge[1]), (edge[2], edge[3]), etc...
