...
SignificanceVertexPartition::create(Graph *graph,
...
double SignificanceVertexPartition::diff_move(size_t v, size_t new_comm) {
#ifdef DEBUG
  cerr << "virtual double SignificanceVertexPartition::diff_move(" << v << ", "
       << new_comm << ")" << endl;
#endif
  ...
  if (new_comm != old_comm) {
    ...
    cerr << "\t"
         << "n: " << n << ", m: " << this->graph->total_weight() << ", p: " << p
         << "." << endl;
#endif
    ...
    // Old comm after move
    size_t n_oldx =
        n_old - nsize; // It should not be possible that this becomes negative,
                       // so no need for ptrdiff_t here.
    size_t N_oldx = this->graph->possible_edges(n_oldx);
    double sw = this->graph->node_self_weight(v);
    // Be careful to exclude the self weight here, because this is include in
    // the weight_to_comm function.
...
#ifdef DEBUG
    cerr << "\t"
         << "n_oldx: " << n_oldx << ", N_oldx: " << N_oldx
         << ", m_oldx: " << m_oldx << ", q_oldx: " << q_oldx
         << ", KL: " << KL(q_oldx, p) << "." << endl;
#endif
...
#ifdef DEBUG
    cerr << "\t"
         << "n_newx: " << n_newx << ", N_newx: " << N_newx
         << ", m_newx: " << m_newx << ", q_newx: " << q_newx
         << ", KL: " << KL(q_newx, p) << "." << endl;
#endif
...
#ifdef DEBUG
  cerr << "exit double SignificanceVertexPartition::diff_move(" << v << ", "
       << new_comm << ")" << endl;
...
#ifdef DEBUG
    cerr << "\t"
         << "c=" << c << ", n_c=" << n_c << ", m_c=" << m_c << ", N_c=" << N_c
         << ", p_c=" << p_c << ", p=" << p << ", KLL=" << KL(p_c, p) << "."
         << endl;
#endif
    S += N_c * KLL(p_c, p);
  }
