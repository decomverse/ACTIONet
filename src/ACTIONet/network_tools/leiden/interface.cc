...
vec signed_cluster(sp_mat A, double resolution_parameter = 1.0,
  ...
  // printf("idx = %d, V = %d, E = %d, total_weight = %f, weighted = %d,
  // directed = %d\n", idx, G->vcount(), G->ecount(), G->total_weight(),
  // G->is_weighted(), G->is_directed());
...
vec unsigned_cluster(sp_mat A, double resolution_parameter = 1.0,
  ...
  if (initial_clusters.n_elem == nV) {
    ...
    partition =
        new RBConfigurationVertexPartition(G, membership, resolution_parameter);
  } else {
