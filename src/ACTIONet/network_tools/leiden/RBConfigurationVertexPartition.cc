...
RBConfigurationVertexPartition::RBConfigurationVertexPartition(
    Graph *graph, vector<size_t> const &membership, double resolution_parameter)
...
RBConfigurationVertexPartition::create(Graph *graph) {
...
RBConfigurationVertexPartition::create(Graph *graph,
  ...
  return new RBConfigurationVertexPartition(graph, membership,
...
#ifdef DEBUG
  cerr << "double RBConfigurationVertexPartition::diff_move(" << v << ", "
       << new_comm << ")" << endl;
#endif
  size_t old_comm = this->_membership[v];
  double diff = 0.0;
  double total_weight =
      this->graph->total_weight() * (2.0 - this->graph->is_directed());
  if (total_weight == 0.0)
    return 0.0;
...
#endif
    double diff_old = (w_to_old - this->resolution_parameter * k_out *
                                      K_in_old / total_weight) +
                      (w_from_old - this->resolution_parameter * k_in *
                                        K_out_old / total_weight);
#ifdef DEBUG
    cerr << "\t"
         << "diff_old: " << diff_old << endl;
#endif
    double diff_new =
        (w_to_new + self_weight -
         this->resolution_parameter * k_out * K_in_new / total_weight) +
        (w_from_new + self_weight -
         this->resolution_parameter * k_in * K_out_new / total_weight);
#ifdef DEBUG
    cerr << "\t"
...
#ifdef DEBUG
  cerr << "exit RBConfigurationVertexPartition::diff_move(" << v << ", "
       << new_comm << ")" << endl;
...
double RBConfigurationVertexPartition::quality(double resolution_parameter) {
  ...
  for (size_t c = 0; c < this->n_communities(); c++) {
    ...
    cerr << "\t"
         << "Comm: " << c << ", size=" << csize << ", w=" << w
         << ", w_out=" << w_out << ", w_in=" << w_in << "." << endl;
#endif
    mod += w - resolution_parameter * w_out * w_in /
                   ((this->graph->is_directed() ? 1.0 : 4.0) *
                    this->graph->total_weight());
  }
