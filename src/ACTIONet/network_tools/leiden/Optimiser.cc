...
*****************************************************************************/
double
Optimiser::optimise_partition(vector<MutableVertexPartition *> partitions,
...
double Optimiser::move_nodes(vector<MutableVertexPartition *> partitions,
  ...
  shuffle(nodes, &rng);
  for (vector<size_t>::iterator it_node = nodes.begin(); it_node != nodes.end();
       it_node++) {
  ...
  while (!vertex_order.empty()) {
    ...
    if (consider_comms == ALL_COMMS) {
      ...
      /****************************ALL NEIGH COMMS*****************************/
      for (size_t layer = 0; layer < nb_layers; layer++) {
        vector<size_t> const &neigh_comm_layer =
    ...
    if (consider_empty_community) {
      for (size_t layer = 0; layer < nb_layers; layer++) {
               ...
               // than nodes)
        {
          ...
          double possible_improv = 0.0;
          for (size_t layer_2 = 0; layer_2 < nb_layers; layer_2++) {
            possible_improv += layer_weights[layer_2] *
                               partitions[layer_2]->diff_move(v, comm);
          }
          if (possible_improv > max_improv) {
            max_improv = possible_improv;
            max_comm = comm;
...
double Optimiser::move_nodes_constrained(
  ...
  while (!vertex_order.empty()) {
    ...
    if (consider_comms == ALL_COMMS) {
      ...
      for (vector<size_t>::const_iterator u_constrained_comm_it =
               constrained_comms[v_constrained_comm].begin();
      ...
      /****************************ALL NEIGH COMMS*****************************/
      for (size_t layer = 0; layer < nb_layers; layer++) {
        set<size_t> neigh_comm_layer = partitions[layer]->get_neigh_comms(
    ...
    if (partitions[0]->cnodes(v_comm) == 1) {
      ...
      if (consider_comms == ALL_COMMS) {
        ...
        for (vector<size_t>::const_iterator u_constrained_comm_it =
                 constrained_comms[v_constrained_comm].begin();
             u_constrained_comm_it !=
             constrained_comms[v_constrained_comm].end();
             u_constrained_comm_it++) {
          size_t u = *u_constrained_comm_it;
          size_t u_comm = partitions[0]->membership(u);
          comms.insert(u_comm);
