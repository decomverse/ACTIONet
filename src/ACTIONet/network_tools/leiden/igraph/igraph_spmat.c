...
CS_INT *cs_amd(CS_INT order,
               const cs *A) /* order 0:natural, 1:Chol, 2:LU, 3:QR */
{
  cs *C, *A2, *AT;
...
static void init_ata(cs *AT, const CS_INT *post, CS_INT *w, CS_INT **head,
                     CS_INT **next) {
...
CS_INT *cs_counts(const cs *A, const CS_INT *parent, const CS_INT *post,
                  CS_INT ata) {
...
CS_INT cs_dfs(CS_INT j, cs *G, CS_INT top, CS_INT *xi, CS_INT *pstack,
              const CS_INT *pinv) {
...
static CS_INT cs_bfs(const cs *A, CS_INT n, CS_INT *wi, CS_INT *wj,
                     CS_INT *queue, const CS_INT *imatch, const CS_INT *jmatch,
                     CS_INT mark) {
  ...
  if (tail == 0)
    return (1); /* quick return if no unmatched nodes */
  C = (mark == 1) ? ((cs *)A) : cs_transpose(A, 0);
  if (!C)
    return (0); /* bfs of C=A' to find R3,C3 from R0 */
  Ap = C->p;
  Ai = C->i;
  ...
  if (mark != 1)
    cs_spfree(C); /* free A' if it was created */
  return (1);
}

/* collect matched rows and columns into p and q */
static void cs_matched(CS_INT n, const CS_INT *wj, const CS_INT *imatch,
                       CS_INT *p, CS_INT *q, CS_INT *cc, CS_INT *rr, CS_INT set,
                       CS_INT mark) {
  CS_INT kc = cc[set], j;
  CS_INT kr = rr[set - 1];
  for (j = 0; j < n; j++) {
...
static void cs_unmatched(CS_INT m, const CS_INT *wi, CS_INT *p, CS_INT *rr,
                         CS_INT set) {
  CS_INT i, kr = rr[set];
  for (i = 0; i < m; i++)
    if (wi[i] == 0)
...
CS_INT cs_ereach(const cs *A, CS_INT k, const CS_INT *parent, CS_INT *s,
                 CS_INT *w) {
...
CS_INT cs_fkeep(cs *A, CS_INT (*fkeep)(CS_INT, CS_INT, CS_ENTRY, void *),
                void *other) {
...
CS_INT cs_leaf(CS_INT i, CS_INT j, const CS_INT *first, CS_INT *maxfirst,
               CS_INT *prevleaf, CS_INT *ancestor, CS_INT *jleaf) {
...
csn *cs_lu(const cs *A, const css *S, double tol) {
  ...
  double a, t;
  CS_INT *Lp, *Li, *Up, *Ui, *pinv, *xi, *q, n, ipiv, k, top, p, i, col, lnz,
      unz;
  if (!CS_CSC(A) || !S)
    return (NULL); /* check inputs */
  n = A->n;
...
/* wrapper for malloc */
void *cs_malloc(CS_INT n, size_t size) { return (malloc(CS_MAX(n, 1) * size)); }

/* wrapper for calloc */
void *cs_calloc(CS_INT n, size_t size) { return (calloc(CS_MAX(n, 1), size)); }
...
static void cs_augment(CS_INT k, const cs *A, CS_INT *jmatch, CS_INT *cheap,
                       CS_INT *w, CS_INT *js, CS_INT *is, CS_INT *ps) {
...
CS_INT *cs_maxtrans(const cs *A,
                    CS_INT seed) /*[jmatch [0..m-1]; imatch [0..n-1]]*/
{
...
cs *cs_permute(const cs *A, const CS_INT *pinv, const CS_INT *q,
               CS_INT values) {
...
/*     if (nz < 0) */
/*     { */
/*         printf (""CS_ID"-by-"CS_ID", nzmax: "CS_ID" nnz: "CS_ID", 1-norm:
 * %g\n", m, n, nzmax, */
/*                 Ap [n], cs_norm (A)) ; */
/*         for (j = 0 ; j < n ; j++) */
/*         { */
...
csn *cs_qr(const cs *A, const css *S) {
  CS_ENTRY *Rx, *Vx, *Ax, *x;
  double *Beta;
  CS_INT i, k, p, m, n, vnz, p1, top, m2, len, col, rnz, *s, *leftmost, *Ap,
...
CS_INT cs_scatter(const cs *A, CS_INT j, CS_ENTRY beta, CS_INT *w, CS_ENTRY *x,
                  CS_INT mark, cs *C, CS_INT nz) {
  CS_INT i, p, *Ap, *Ai, *Ci;
  CS_ENTRY *Ax;
...
CS_INT cs_spsolve(cs *G, const cs *B, CS_INT k, CS_INT *xi, CS_ENTRY *x,
...
CS_INT cs_tdfs(CS_INT j, CS_INT k, CS_INT *head, const CS_INT *next,
               CS_INT *post, CS_INT *stack) {
  CS_INT i, p, top = 0;
...
cs *cs_spalloc(CS_INT m, CS_INT n, CS_INT nzmax, CS_INT values,
               CS_INT triplet) {
...
CS_INT cs_sprealloc(cs *A, CS_INT nzmax) {
  ...
  if (CS_TRIPLET(A))
    A->p = cs_realloc(A->p, nzmax, sizeof(CS_INT), &okj);
  if (A->x)
    A->x = cs_realloc(A->x, nzmax, sizeof(CS_ENTRY), &okx);
  ok = (oki && okj && okx);
  if (ok)
    A->nzmax = nzmax;
  return (ok);
