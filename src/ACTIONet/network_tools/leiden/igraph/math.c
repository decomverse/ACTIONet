...
double igraph_log2(const double a) { return log(a) / log(2.0); }
...
double igraph_log1p(double x) {
  ...
  if (nlnrel == 0) { /* initialize chebychev coefficients */
    nlnrel = igraph_chebyshev_init(alnrcs, 43,
                                   DBL_EPSILON / 20); /*was .1*d1mach(3)*/
  }
...
int igraph_is_nan(double x) { return isnan(x); }

int igraph_is_inf(double x) { return isinf(x) != 0; }

int igraph_is_posinf(double x) { return isinf(x) == 1; }

int igraph_is_neginf(double x) { return isinf(x) == -1; }
