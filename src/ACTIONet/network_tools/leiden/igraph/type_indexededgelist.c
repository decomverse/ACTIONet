...
static int igraph_i_create_start(igraph_vector_t *res, igraph_vector_t *el,
 ...
 * \brief Creates an empty graph with some vertices, no edges and some graph
 * attributes.
 *
 * </para><para>
...
int igraph_delete_vertices_idx(igraph_t *graph, const igraph_vs_t vertices,
                               igraph_vector_t *idx, igraph_vector_t *invidx) {
  ...
  IGRAPH_CHECK(igraph_i_create_start(&newgraph.os, &newgraph.from, &newgraph.oi,
                                     (igraph_integer_t)remaining_vertices));
  IGRAPH_CHECK(igraph_i_create_start(&newgraph.is, &newgraph.to, &newgraph.ii,
                                     (igraph_integer_t)remaining_vertices));
 ...
 * Time complexity: O(1)
 */
igraph_integer_t igraph_vcount(const igraph_t *graph) { return graph->n; }
  ...
  if (!igraph_is_directed(graph) || mode != IGRAPH_ALL) {
    ...
    if (mode & IGRAPH_OUT) {
      ...
      for (i = (long int)VECTOR(graph->os)[node]; i < j; i++) {
        VECTOR(*neis)
        [idx++] = VECTOR(graph->to)[(long int)VECTOR(graph->oi)[i]];
      }
    }
    if (mode & IGRAPH_IN) {
      ...
      for (i = (long int)VECTOR(graph->is)[node]; i < j; i++) {
        VECTOR(*neis)
        [idx++] = VECTOR(graph->from)[(long int)VECTOR(graph->ii)[i]];
      }
    }
  } else {
    ...
    while (i1 < jj1 && i2 < j2) {
      long int n1 =
          (long int)VECTOR(graph->to)[(long int)VECTOR(graph->oi)[i1]];
      long int n2 =
          (long int)VECTOR(graph->from)[(long int)VECTOR(graph->ii)[i2]];
      if (n1 < n2) {
        VECTOR(*neis)[idx++] = n1;
        i1++;
      } else if (n1 > n2) {
 ...
 * \brief Query edge ids based on their adjacent vertices, handle multiple
 * edges.
 *
 * This function operates in two modes. If the \c pairs argument is
 * not a null pointer, but the \c path argument is, then it searches
