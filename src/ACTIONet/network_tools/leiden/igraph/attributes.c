...
int igraph_i_attribute_combine_vertices(
  ...
  if (igraph_i_attribute_table) {
    return igraph_i_attribute_table->combine_vertices(graph, newgraph, merges,
                                                      comb);
  } else {
    return 0;
  }
...
int igraph_i_attribute_combine_edges(
    ...
    const igraph_attribute_combination_t *comb) {
  if (igraph_i_attribute_table) {
    return igraph_i_attribute_table->combine_edges(graph, newgraph, merges,
                                                   comb);
  } else {
    return 0;
  }
...
int igraph_i_attribute_get_info(
  ...
  if (igraph_i_attribute_table) {
    return igraph_i_attribute_table->get_info(graph, gnames, gtypes, vnames,
                                              vtypes, enames, etypes);
  } else {
    return 0;
  }
}
...
int igraph_i_attribute_get_numeric_vertex_attr(const igraph_t *graph,
                                               ...
                                               igraph_vector_t *value) {
  if (igraph_i_attribute_table) {
    return igraph_i_attribute_table->get_numeric_vertex_attr(graph, name, vs,
                                                             value);
  } else {
    return 0;
  }
...
int igraph_i_attribute_get_numeric_edge_attr(const igraph_t *graph,
                                             ...
                                             igraph_vector_t *value) {
  if (igraph_i_attribute_table) {
    return igraph_i_attribute_table->get_numeric_edge_attr(graph, name, es,
                                                           value);
  } else {
    return 0;
  }
...
int igraph_i_attribute_get_string_vertex_attr(const igraph_t *graph,
                                              ...
                                              igraph_strvector_t *value) {
  if (igraph_i_attribute_table) {
    return igraph_i_attribute_table->get_string_vertex_attr(graph, name, vs,
                                                            value);
  } else {
    return 0;
  }
...
int igraph_i_attribute_get_string_edge_attr(const igraph_t *graph,
                                            ...
                                            igraph_strvector_t *value) {
  if (igraph_i_attribute_table) {
    return igraph_i_attribute_table->get_string_edge_attr(graph, name, es,
                                                          value);
  } else {
    return 0;
  }
...
int igraph_i_attribute_get_bool_vertex_attr(const igraph_t *graph,
                                            ...
                                            igraph_vector_bool_t *value) {
  if (igraph_i_attribute_table) {
    return igraph_i_attribute_table->get_bool_vertex_attr(graph, name, vs,
                                                          value);
  } else {
    return 0;
  }
...
void igraph_attribute_combination_destroy(
...
int igraph_attribute_combination_add(igraph_attribute_combination_t *comb,
  ...
  for (i = 0; i < n; i++) {
    ...
    const char *n = r->name;
    if ((!name && !n) || (name && n && !strcmp(n, name))) {
      r->type = type;
      r->func = func;
      break;
    }
  ...
  if (i == n) {
    ...
    if (!rec) {
      IGRAPH_ERROR("Cannot create attribute combination data", IGRAPH_ENOMEM);
    }
    if (!name) {
      rec->name = 0;
    } else {
...
int igraph_attribute_combination_query(
