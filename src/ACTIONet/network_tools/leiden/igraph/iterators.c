...
int igraph_vs_adj(igraph_vs_t *vs, igraph_integer_t vid,
...
int igraph_vs_vector_copy(igraph_vs_t *vs, const igraph_vector_t *v) {
  vs->type = IGRAPH_VS_VECTOR;
  vs->data.vecptr = igraph_Calloc(1, igraph_vector_t);
  if (vs->data.vecptr == 0) {
    IGRAPH_ERROR("Cannot create vertex selector", IGRAPH_ENOMEM);
...
int igraph_vs_size(const igraph_t *graph, const igraph_vs_t *vs,
  ...
  case IGRAPH_VS_ADJ:
    ...
    IGRAPH_CHECK(
        igraph_neighbors(graph, &vec, vs->data.adj.vid, vs->data.adj.mode));
  ...
  case IGRAPH_VS_VECTOR:
  case IGRAPH_VS_VECTORPTR:
    *result = (igraph_integer_t)igraph_vector_size(
        (igraph_vector_t *)vs->data.vecptr);
    return 0;
  }
...
int igraph_vit_create(const igraph_t *graph, igraph_vs_t vs,
                      igraph_vit_t *vit) {
  igraph_vector_t vec;
  igraph_bool_t *seen;
  long int i, j, n;
 ...
 */
int igraph_es_adj(igraph_es_t *es, igraph_integer_t vid,
                  igraph_neimode_t mode) {
...
int igraph_es_incident(igraph_es_t *es, igraph_integer_t vid,
                       igraph_neimode_t mode) {
 ...
 * edge selection. The vertex pairs must be defined in the vector
 * <code>v</code>, the first element of the vector is the first vertex of the
 * first edge to be selected, the second element is the second vertex of the
 * first edge, the third element is the first vertex of the second edge and so
 * on.
 *
 * \param es Pointer to an uninitialized edge selector object.
...
static int igraph_i_es_pairs_size(const igraph_t *graph, const igraph_es_t *es,
                                  igraph_integer_t *result);
static int igraph_i_es_path_size(const igraph_t *graph, const igraph_es_t *es,
                                 igraph_integer_t *result);
...
int igraph_es_size(const igraph_t *graph, const igraph_es_t *es,
  ...
  case IGRAPH_ES_INCIDENT:
    ...
    IGRAPH_CHECK(igraph_incident(graph, &v, es->data.incident.vid,
                                 es->data.incident.mode));
  ...
  case IGRAPH_ES_VECTORPTR:
    *result = (igraph_integer_t)igraph_vector_size(
        (igraph_vector_t *)es->data.vecptr);
    return 0;
...
static int igraph_i_es_pairs_size(const igraph_t *graph, const igraph_es_t *es,
  ...
  if (n % 2 != 0) {
    IGRAPH_ERROR(
        "Cannot calculate edge selector length from odd number of vertices",
        IGRAPH_EINVAL);
  }
...
static int igraph_i_eit_create_allfromto(const igraph_t *graph,
                                         igraph_eit_t *eit,
                                         igraph_neimode_t mode);
static int igraph_i_eit_pairs(const igraph_t *graph, igraph_es_t es,
                              igraph_eit_t *eit);
static int igraph_i_eit_multipairs(const igraph_t *graph, igraph_es_t es,
                                   igraph_eit_t *eit);
static int igraph_i_eit_path(const igraph_t *graph, igraph_es_t es,
                             igraph_eit_t *eit);
...
int igraph_eit_create(const igraph_t *graph, igraph_es_t es,
                      igraph_eit_t *eit) {
  switch (es.type) {
  case IGRAPH_ES_ALL:
