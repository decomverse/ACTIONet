...
int igraph_indheap_init_array(igraph_indheap_t *h, igraph_real_t *data,
                              long int len) {
  long int i;
...
void igraph_indheap_i_sink(igraph_indheap_t *h, long int head) {
  ...
  if (LEFTCHILD(head) >= size) {
    /* no subtrees */
  } else if (RIGHTCHILD(head) == size || h->stor_begin[LEFTCHILD(head)] >=
                                             h->stor_begin[RIGHTCHILD(head)]) {
...
void igraph_d_indheap_i_switch(igraph_d_indheap_t *h, long int e1,
                               long int e2) {
  if (e1 != e2) {
    long int tmpi;
    igraph_real_t tmp = h->stor_begin[e1];
...
void igraph_i_cutheap_switch(igraph_i_cutheap_t *ch, long int hidx1,
                             long int hidx2) {
  if (hidx1 != hidx2) {
...
int igraph_i_cutheap_init(igraph_i_cutheap_t *ch, igraph_integer_t nodes) {
  ...
  IGRAPH_CHECK(
      igraph_vector_init_seq(&ch->hptr, INDEXINC, nodes + INDEXINC - 1));
  IGRAPH_FINALLY_CLEAN(2);
  return 0;
}
...
void igraph_i_2wheap_switch(igraph_2wheap_t *h, long int e1, long int e2) {
  if (e1 != e2) {
    long int tmp1, tmp2;
...
void igraph_i_2wheap_shift_up(igraph_2wheap_t *h, long int elem) {
...
void igraph_i_2wheap_sink(igraph_2wheap_t *h, long int head) {
  long int size = igraph_2wheap_size(h);
  if (LEFTCHILD(head) >= size) {
    /* no subtrees */
  } else if (RIGHTCHILD(head) == size ||
             VECTOR(h->data)[LEFTCHILD(head)] >=
                 VECTOR(h->data)[RIGHTCHILD(head)]) {
    /* sink to the left if needed */
...
long int igraph_2wheap_max_size(const igraph_2wheap_t *h) { return h->size; }
...
igraph_real_t igraph_2wheap_delete_max_index(igraph_2wheap_t *h,
                                             long int *idx) {
