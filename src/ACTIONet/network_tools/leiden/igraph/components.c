...
static int igraph_i_clusters_weak(const igraph_t *graph,
                                  igraph_vector_t *membership,
                                  igraph_vector_t *csize, igraph_integer_t *no);

static int igraph_i_clusters_strong(const igraph_t *graph,
                                    igraph_vector_t *membership,
...
static int igraph_i_clusters_weak(const igraph_t *graph,
  ...
  IGRAPH_DQUEUE_INIT_FINALLY(&q,
                             no_of_nodes > 100000 ? 10000 : no_of_nodes / 10);
  ...
  for (first_node = 0; first_node < no_of_nodes; ++first_node) {
    ...
    while (!igraph_dqueue_empty(&q)) {
      ...
      IGRAPH_CHECK(igraph_neighbors(graph, &neis, (igraph_integer_t)act_node,
                                    IGRAPH_ALL));
  ...
  for (i = 0; i < no_of_nodes; i++) {
    ...
    while (!igraph_dqueue_empty(&q)) {
      ...
      if (VECTOR(next_nei)[act_node] == 0) {
        ...
        long int neighbor =
            (long int)VECTOR(*tmp)[(long int)VECTOR(next_nei)[act_node] - 1];
...
int igraph_is_connected_weak(const igraph_t *graph, igraph_bool_t *res) {
  ...
  while (!igraph_dqueue_empty(&q)) {
    ...
    IGRAPH_ALLOW_INTERRUPTION();
    IGRAPH_CHECK(
        igraph_neighbors(graph, &neis, (igraph_integer_t)actnode, IGRAPH_ALL));
    for (i = 0; i < igraph_vector_size(&neis); i++) {
      long int neighbor = (long int)VECTOR(neis)[i];
...
static int igraph_i_decompose_weak(const igraph_t *graph,
  ...
  for (actstart = 0; resco < maxcompno && actstart < no_of_nodes; actstart++) {
    ...
    IGRAPH_CHECK(igraph_induced_subgraph(graph, newg, igraph_vss_vector(&verts),
 ...
 *
 * \sa \ref igraph_biconnected_components(), \ref igraph_clusters(), \ref
 * igraph_bridges()
 */
...
int igraph_biconnected_components(const igraph_t *graph, igraph_integer_t *no,
              ...
              if (component_edges) {
                ...
                for (ii = 0; ii < no_vert; ii++) {
                  ...
                  igraph_vector_int_t *edges =
                      igraph_inclist_get(&inclist, vert);
...
static int
igraph_i_bridges_rec(const igraph_t *graph, const igraph_inclist_t *il,
                     igraph_integer_t u, igraph_integer_t *time,
                     igraph_vector_t *bridges, igraph_vector_bool_t *visited,
                     igraph_vector_int_t *disc, igraph_vector_int_t *low,
  ...
  *time += 1;
  ...
  for (i = 0; i < nc; ++i) {
    ...
    igraph_integer_t v = IGRAPH_TO(graph, edge) == u ? IGRAPH_FROM(graph, edge)
                                                     : IGRAPH_TO(graph, edge);

    if (!VECTOR(*visited)[v]) {
      VECTOR(*incoming_edge)[v] = edge;
      IGRAPH_CHECK(igraph_i_bridges_rec(graph, il, v, time, bridges, visited,
                                        disc, low, incoming_edge));

      VECTOR(*low)
      [u] =
          VECTOR(*low)[u] < VECTOR(*low)[v] ? VECTOR(*low)[u] : VECTOR(*low)[v];
      ...
      VECTOR(*low)
      [u] = VECTOR(*low)[u] < VECTOR(*disc)[v] ? VECTOR(*low)[u]
                                               : VECTOR(*disc)[v];
    }
  }
...
int igraph_bridges(const igraph_t *graph, igraph_vector_t *bridges) {
  ...
  for (i = 0; i < n; ++i)
    if (!VECTOR(visited)[i]) {
      IGRAPH_CHECK(igraph_i_bridges_rec(graph, &il, i, &time, bridges, &visited,
                                        &disc, &low, &incoming_edge));
    }
