...
int igraph_get_edgelist(const igraph_t *graph, igraph_vector_t *res,
                        igraph_bool_t bycol) {
  ...
  IGRAPH_CHECK(
      igraph_eit_create(graph, igraph_ess_all(IGRAPH_EDGEORDER_ID), &edgeit));
  IGRAPH_FINALLY(igraph_eit_destroy, &edgeit);
  ...
  if (mode == IGRAPH_TO_UNDIRECTED_EACH) {
    ...
    *graph = newgraph;
    ...
    for (i = 0; i < no_of_nodes; i++) {
      ...
      IGRAPH_CHECK(
          igraph_incident(graph, &outadj, (igraph_integer_t)i, IGRAPH_OUT));
      IGRAPH_CHECK(
          igraph_incident(graph, &inadj, (igraph_integer_t)i, IGRAPH_IN));
...
#undef STEPIN
    ...
    if (attr) {
      ...
      IGRAPH_CHECK(
          igraph_i_attribute_combine_edges(graph, &newgraph, &vl.v, edge_comb));
...
int igraph_get_stochastic(const igraph_t *graph, igraph_matrix_t *matrix,
 ...
 * \param prufer A pointer to the integer vector that should hold the
 Pr&uuml;fer sequence; the vector must be initialized and will be resized to n
 - 2.
 * \return Error code:
 *          \clist
 *          \cli IGRAPH_ENOMEM
...
int igraph_to_prufer(const igraph_t *graph, igraph_vector_int_t *prufer) {
  /* For generating the Prüfer sequence, we enumerate the vertices u of the
     tree. We keep track of the degrees of all vertices, treating vertices of
     degree 0 as removed. We maintain the invariant that all leafs that are
     still contained in the tree are >= u. If u is a leaf, we remove it and add
     its unique neighbor to the prüfer sequence. If the removal of u turns the
     neighbor into a leaf which is < u,
  ...
  IGRAPH_CHECK(igraph_degree(graph, &degrees, igraph_vss_all(), IGRAPH_ALL,
                             IGRAPH_NO_LOOPS));
