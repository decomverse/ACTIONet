...
int igraph_sparsemat_init(igraph_sparsemat_t *A, int rows, int cols,
                          int nzmax) {
...
void igraph_sparsemat_destroy(igraph_sparsemat_t *A) { cs_spfree(A->cs); }
...
long int igraph_sparsemat_nrow(const igraph_sparsemat_t *A) { return A->cs->m; }
...
long int igraph_sparsemat_ncol(const igraph_sparsemat_t *A) { return A->cs->n; }
...
static int igraph_i_sparsemat_index_rows(const igraph_sparsemat_t *A,
  ...
  /* Create index matrix */
  IGRAPH_CHECK(
      igraph_sparsemat_init(&II2, (int)idx_rows, (int)nrow, (int)idx_rows));
...
static int igraph_i_sparsemat_index_cols(const igraph_sparsemat_t *A,
  ...
  /* Create index matrix */
  IGRAPH_CHECK(
      igraph_sparsemat_init(&JJ2, (int)ncol, (int)idx_cols, (int)idx_cols));
...
int igraph_sparsemat_entry(igraph_sparsemat_t *A, int row, int col,
  ...
  if (!cs_entry(A->cs, row, col, elem)) {
    IGRAPH_ERROR("Cannot add entry to sparse matrix", IGRAPH_FAILURE);
  }
...
igraph_i_sparsemat_is_symmetric_cc(const igraph_sparsemat_t *A) {
  ...
  res = res &&
        memcmp(t.cs->p, tt.cs->p, sizeof(int) * (size_t)(t.cs->n + 1)) == 0;
  res =
      res && memcmp(t.cs->x, tt.cs->x, sizeof(igraph_real_t) * (size_t)nz) == 0;
...
int igraph_sparsemat_cholsol(const igraph_sparsemat_t *A,
  ...
  if (A->cs->m != A->cs->n) {
    IGRAPH_ERROR("Cannot perform sparse symmetric solve", IGRAPH_NONSQUARE);
  }
...
int igraph_sparsemat_lusol(const igraph_sparsemat_t *A,
  ...
  if (A->cs->m != A->cs->n) {
    IGRAPH_ERROR("Cannot perform LU solve", IGRAPH_NONSQUARE);
  }
...
static int igraph_i_sparsemat_triplet(igraph_t *graph,
...
static int igraph_i_weighted_sparsemat_cc(const igraph_sparsemat_t *A,
                                          igraph_bool_t directed,
                                          const char *attr, igraph_bool_t loops,
                                          igraph_vector_t *edges,
...
static int igraph_i_weighted_sparsemat_triplet(
    const igraph_sparsemat_t *A, igraph_bool_t directed, const char *attr,
    igraph_bool_t loops, igraph_vector_t *edges, igraph_vector_t *weights) {
  ...
  /* TODO */
  IGRAPH_ERROR("Triplet matrices are not implemented", IGRAPH_UNIMPLEMENTED);
  return 0;
}
...
int igraph_weighted_sparsemat(igraph_t *graph, const igraph_sparsemat_t *A,
  ...
  if (A->cs->nz < 0) {
    ...
    IGRAPH_CHECK(igraph_i_weighted_sparsemat_triplet(A, directed, attr, loops,
                                                     &edges, &weights));
  }
...
#define CHECK(x)                                                               \
  if ((x) < 0) {                                                               \
    IGRAPH_ERROR("Cannot write to file", IGRAPH_EFILE);                        \
  }
...
static int igraph_i_sparsemat_eye_triplet(igraph_sparsemat_t *A, int n,
                                          int nzmax, igraph_real_t value) {
  long int i;
...
static int igraph_i_sparsemat_arpack_multiply(igraph_real_t *to,
                                              const igraph_real_t *from, int n,
                                              void *extra) {
...
int igraph_sparsemat_arpack_rssolve(const igraph_sparsemat_t *A,
  ...
  if (options->mode == 1) {
    ...
    // (void*) A, options, storage, values, vectors));
    ...
    // (void*) &data, options, storage, values, vectors));
...
int igraph_sparsemat_arpack_rnsolve(const igraph_sparsemat_t *A,
  ...
  return -1; // igraph_arpack_rnsolve(igraph_i_sparsemat_arpack_multiply,
             // (void*) A, options, storage, values, vectors);
}
...
int igraph_sparsemat_qrresol(const igraph_sparsemat_symbolic_t *dis,
  ...
  workspace =
      igraph_Calloc(dis->symbolic ? dis->symbolic->m2 : 1, igraph_real_t);
  if (!workspace) {
...
static int
igraph_i_sparsemat_as_matrix_triplet(igraph_matrix_t *res,
                                     const igraph_sparsemat_t *spmat) {
  int nrow = (int)igraph_sparsemat_nrow(spmat);
  int ncol = (int)igraph_sparsemat_ncol(spmat);
  int *i = spmat->cs->p;
...
int igraph_sparsemat_nzmax(const igraph_sparsemat_t *A) { return A->cs->nzmax; }
...
int igraph_sparsemat_iterator_reset(igraph_sparsemat_iterator_t *it) {
  ...
  if (!igraph_sparsemat_is_triplet(it->mat)) {
    it->col = 0;
    while (it->col < it->mat->cs->n && it->mat->cs->p[it->col + 1] == it->pos) {
      it->col++;
    }
  }
  return 0;
...
igraph_sparsemat_iterator_end(const igraph_sparsemat_iterator_t *it) {
  int nz =
      it->mat->cs->nz == -1 ? it->mat->cs->p[it->mat->cs->n] : it->mat->cs->nz;
  return it->pos >= nz;
}
