...
void igraph_qsort_r(void *a, size_t n, size_t es, void *thunk,
                    int (*cmp)(void *, const void *, const void *)) {
...
void igraph_qsort(void *a, size_t n, size_t es,
                  int (*cmp)(void *, const void *, const void *)) {
...
int igraph_diameter(const igraph_t *graph, igraph_integer_t *pres,
                    ...
                    igraph_vector_t *path, igraph_bool_t directed,
  ...
  if (path != 0) {
    if (res == no_of_nodes) {
      ...
      IGRAPH_CHECK(igraph_get_shortest_paths(
          graph, &tmpptr, 0, (igraph_integer_t)from,
          igraph_vss_1((igraph_integer_t)to), dirmode, 0, 0));
 ...
 *        The rationale behind this is that this is always longer than the
 * longest possible geodesic in a graph. \return Error code: \c IGRAPH_ENOMEM,
...
int igraph_shortest_paths(const igraph_t *graph, igraph_matrix_t *res,
  ...
  for (IGRAPH_VIT_RESET(fromvit), i = 0; !IGRAPH_VIT_END(fromvit);
...
int igraph_get_shortest_paths(const igraph_t *graph,
                              ...
                              igraph_vector_ptr_t *edges, igraph_integer_t from,
                              const igraph_vs_t to, igraph_neimode_t mode,
  ...
  if (vertices && IGRAPH_VIT_SIZE(vit) != igraph_vector_ptr_size(vertices)) {
    IGRAPH_ERROR("Size of the `vertices' and the `to' should match",
                 IGRAPH_EINVAL);
  }
  if (edges && IGRAPH_VIT_SIZE(vit) != igraph_vector_ptr_size(edges)) {
    IGRAPH_ERROR("Size of the `edges' and the `to' should match",
                 IGRAPH_EINVAL);
  }
  ...
  if (vertices || edges) {
    for (IGRAPH_VIT_RESET(vit), j = 0; !IGRAPH_VIT_END(vit);
...
int igraph_get_shortest_path(const igraph_t *graph, igraph_vector_t *vertices,
                             igraph_vector_t *edges, igraph_integer_t from,
                             igraph_integer_t to, igraph_neimode_t mode) {
  ...
  IGRAPH_CHECK(igraph_get_shortest_paths(graph, vp, ep, from, igraph_vss_1(to),
                                         mode, 0, 0));
 ...
 * \brief Finds all shortest paths (geodesics) from a vertex to all other
 * vertices.
 *
 * \param graph The graph object.
...
int igraph_get_all_shortest_paths(const igraph_t *graph,
                                  ...
                                  igraph_vector_t *nrgeo, igraph_integer_t from,
                                  const igraph_vs_t to, igraph_neimode_t mode) {
  ...
  while (!igraph_dqueue_empty(&q)) {
    ...
    if (reached >= to_reach) {
      ...
      if (actdist > maxdist) {
        /* safety check, maxdist should have been set when we reached the last
         * node */
        if (maxdist < 0) {
    ...
    for (j = 0; j < n; j++) {
      ...
      if (geodist[neighbor] > 0 && geodist[neighbor] - 1 < actdist + 1) {
      ...
      while (fatherptr != 0) {
        ...
        IGRAPH_CHECK(
            igraph_vector_push_back(&ptrlist, VECTOR(ptrhead)[neighbor]));
...
int igraph_subcomponent(const igraph_t *graph, igraph_vector_t *res,
                        igraph_real_t vertex, igraph_neimode_t mode) {
  ...
  while (!igraph_dqueue_empty(&q)) {
    ...
    IGRAPH_CHECK(
        igraph_neighbors(graph, &tmp, (igraph_integer_t)actnode, mode));
...
/* Not declared static so that the testsuite can use it, but not part of the
 * public API. */
int igraph_rewire_core(igraph_t *graph, igraph_integer_t n,
                       igraph_rewiring_t mode, igraph_bool_t use_adjlist) {
  long int no_of_nodes = igraph_vcount(graph);
  ...
  while (num_swaps < n) {
    ...
    if (num_swaps % 1000 == 0) {
      snprintf(message, sizeof(message),
               ...
               num_swaps > 0 ? ((100.0 * num_successful_swaps) / num_swaps)
                             : 0.0);
    ...
    case IGRAPH_REWIRING_SIMPLE_LOOPS:
      ...
      if (use_adjlist) {
        ...
        IGRAPH_CHECK(
            igraph_edge(graph, (igraph_integer_t)VECTOR(eids)[0], &a, &b));
        IGRAPH_CHECK(
            igraph_edge(graph, (igraph_integer_t)VECTOR(eids)[1], &c, &d));
      }
 ...
 * flags: \clist \cli IGRAPH_REWIRING_SIMPLE Simple rewiring algorithm which
 * chooses two arbitrary edges in each step (namely (a,b) and (c,d)) and
 * substitutes them with (a,d) and (c,b) if they don't exist.  The method will
...
int igraph_i_subgraph_copy_and_delete(const igraph_t *graph, igraph_t *res,
  ...
  IGRAPH_CHECK(
      igraph_vector_reserve(&_delete_, no_of_nodes - IGRAPH_VIT_SIZE(vit)));
...
int igraph_i_subgraph_create_from_scratch(const igraph_t *graph, igraph_t *res,
  ...
  /* The order of initialization is important here, they will be destroyed in
   * the opposite order */
  ...
  for (i = 0; i < no_of_new_nodes; i++) {
    ...
    if (directed) {
      for (j = 0; j < n; j++) {
        ...
        to =
            (long int)VECTOR(*my_vids_old2new)[(long int)IGRAPH_TO(graph, eid)];
        if (!to) {
          continue;
        }
      ...
      for (j = 0; j < n; j++) {
        ...
        if (IGRAPH_FROM(graph, eid) !=
            old_vid) { /* avoid processing edges twice */
          continue;
        }
  ...
  IGRAPH_CHECK(igraph_create(res, &new_edges, (igraph_integer_t)no_of_new_nodes,
                             directed));
  ...
  /* Make sure that the newly created graph is destroyed if something happens
   * from now on */
  IGRAPH_FINALLY(igraph_destroy, res);
  ...
  IGRAPH_CHECK(
      igraph_i_attribute_permute_vertices(graph, res, my_vids_new2old));
...
int igraph_induced_subgraph_map(const igraph_t *graph, igraph_t *res,
  ...
  if (impl == IGRAPH_SUBGRAPH_AUTO) {
    IGRAPH_CHECK(
        igraph_i_induced_subgraph_suggest_implementation(graph, vids, &impl));
  }
  ...
  case IGRAPH_SUBGRAPH_CREATE_FROM_SCRATCH:
    return igraph_i_subgraph_create_from_scratch(graph, res, vids, map, invmap);
...
int igraph_subgraph_edges(const igraph_t *graph, igraph_t *res,
  ...
  IGRAPH_CHECK(
      igraph_vector_reserve(&_delete_, no_of_edges - IGRAPH_EIT_SIZE(eit)));
...
int igraph_simplify(igraph_t *graph, igraph_bool_t multiple,
  ...
  if (attr) {
    ...
    IGRAPH_CHECK(
        igraph_i_attribute_combine_edges(graph, &res, &vl.v, edge_comb));
...
int igraph_reciprocity(const igraph_t *graph, igraph_real_t *res,
                       igraph_bool_t ignore_loops, igraph_reciprocity_t mode) {
  ...
  if (mode != IGRAPH_RECIPROCITY_DEFAULT && mode != IGRAPH_RECIPROCITY_RATIO) {
...
int igraph_constraint(const igraph_t *graph, igraph_vector_t *res,
  ...
  for (a = 0; a < nodes_to_calc; a++, IGRAPH_VIT_NEXT(vit)) {
    ...
    /* get neighbors of i */
    IGRAPH_CHECK(
        igraph_incident(graph, &ineis_in, (igraph_integer_t)i, IGRAPH_IN));
    IGRAPH_CHECK(
        igraph_incident(graph, &ineis_out, (igraph_integer_t)i, IGRAPH_OUT));
    ...
    for (b = 0; b < igraph_vector_size(&ineis_in); b++) {
      ...
      if (i != j) { /* excluding loops */
        if (weights) {
          VECTOR(contrib)
          [j] += VECTOR(*weights)[(long int)edge] / VECTOR(degree)[i];
        } else {
    ...
    for (b = 0; b < igraph_vector_size(&ineis_in); b++) {
      ...
      IGRAPH_CHECK(
          igraph_incident(graph, &jneis_in, (igraph_integer_t)j, IGRAPH_IN));
      IGRAPH_CHECK(
          igraph_incident(graph, &jneis_out, (igraph_integer_t)j, IGRAPH_OUT));
      for (c = 0; c < igraph_vector_size(&jneis_in); c++) {
        ...
        if (j != q) {
          if (weights) {
            VECTOR(contrib)
            [q] += VECTOR(*weights)[(long int)edge] *
                   VECTOR(*weights)[(long int)edge2] / VECTOR(degree)[i] /
                   VECTOR(degree)[j];
          } else {
...
int igraph_neighborhood_graphs(const igraph_t *graph, igraph_vector_ptr_t *res,
  ...
  for (i = 0; !IGRAPH_VIT_END(vit); IGRAPH_VIT_NEXT(vit), i++) {
    ...
    if (igraph_vector_size(&tmp) < no_of_nodes) {
      IGRAPH_CHECK(igraph_induced_subgraph(graph, newg, igraph_vss_vector(&tmp),
...
int igraph_topological_sorting(const igraph_t *graph, igraph_vector_t *res,
  ...
  if (mode == IGRAPH_ALL || !igraph_is_directed(graph)) {
    IGRAPH_ERROR(
        "topological sorting does not make sense for undirected graphs",
        IGRAPH_EINVAL);
...
int igraph_is_dag(const igraph_t *graph, igraph_bool_t *res) {
  ...
  while (!igraph_dqueue_empty(&sources)) {
    ...
    /* Get the neighbors and decrease their degrees by one */
    IGRAPH_CHECK(
        igraph_neighbors(graph, &neis, (igraph_integer_t)node, IGRAPH_IN));
    j = igraph_vector_size(&neis);
    for (i = 0; i < j; i++) {
 ...
 * \param es The edges to check, for all edges supply \ref igraph_ess_all()
 * here. \return Error code.
 *
 * \sa \ref igraph_simplify() to get rid of loop edges.
 *
 * Time complexity: O(e), the number of edges to check.
 ...
 * \return Error code.
 *
 * \sa \ref igraph_count_multiple(), \ref igraph_is_multiple() and \ref
 * igraph_simplify().
 *
 ...
 * \return Error code.
 *
 * \sa \ref igraph_count_multiple(), \ref igraph_has_multiple() and \ref
 * igraph_simplify().
 *
 * Time complexity: O(e*d), e is the number of edges to check and d is the
 * average degree (out-degree in directed graphs) of the vertices at the
...
int igraph_count_multiple(const igraph_t *graph, igraph_vector_t *res,
                          igraph_es_t es) {
  igraph_eit_t eit;
  long int i;
  igraph_lazy_inclist_t inclist;
...
int igraph_i_linegraph_undirected(const igraph_t *graph, igraph_t *linegraph) {
  ...
  for (i = 0; i < no_of_edges; i++) {
    ...
    IGRAPH_CHECK(
        igraph_incident(graph, &adjedges2, (igraph_integer_t)to, IGRAPH_ALL));
...
int igraph_i_linegraph_directed(const igraph_t *graph, igraph_t *linegraph) {
  ...
  for (i = 0; i < no_of_edges; i++) {
    ...
    if (from != prev) {
      IGRAPH_CHECK(
          igraph_incident(graph, &adjedges, (igraph_integer_t)from, IGRAPH_IN));
    }
    n = igraph_vector_size(&adjedges);
    for (j = 0; j < n; j++) {
      long int e = (long int)VECTOR(adjedges)[j];
...
int igraph_add_edge(igraph_t *graph, igraph_integer_t from,
                    igraph_integer_t to) {
...
int igraph_convergence_degree(const igraph_t *graph, igraph_vector_t *result,
  ...
  if (result != 0) {
    for (i = 0; i < no_of_edges; i++)
      VECTOR(*result)
      [i] = (VECTOR(*ins_p)[i] - VECTOR(*outs_p)[i]) /
...
int igraph_shortest_paths_dijkstra(const igraph_t *graph, igraph_matrix_t *res,
                                   const igraph_vs_t from, const igraph_vs_t to,
    ...
    while (!igraph_2wheap_empty(&Q)) {
      ...
      if (all_to) {
        ...
        if (VECTOR(indexv)[minnei]) {
          MATRIX(*res, i, (long int)(VECTOR(indexv)[minnei] - 1)) =
              mindist - 1.0;
          reached++;
          if (reached == no_of_to) {
            igraph_2wheap_clear(&Q);
 ...
 * this function gives only one of them. \param graph The graph object. \param
 * vertices The result, the ids of the vertices along the paths. This is a
 * pointer vector, each element points to a vector object. These should be
 * initialized before passing them to the function, which will properly clear
 * and/or resize them and fill the ids of the vertices along the geodesics
 * from/to the vertices. Supply a null pointer here if you don't need these
 * vectors. Normally, either this argument, or the \c edges should be non-null,
 * but no error or warning is given if they are both null pointers. \param edges
 * The result, the ids of the edges along the paths. This is a pointer vector,
 * each element points to a vector object. These should be initialized before
 * passing them to the function, which will properly clear and/or resize them
 *        and fill the ids of the vertices along the geodesics from/to
 *        the vertices. Supply a null pointer here if you don't need
 ...
 *
 * \sa \ref igraph_shortest_paths_dijkstra() if you only need the path length
 * but not the paths themselves, \ref igraph_get_shortest_paths() if all edge
 * weights are equal.
 *
 * \example examples/simple/igraph_get_shortest_paths_dijkstra.c
 */
int igraph_get_shortest_paths_dijkstra(
    const igraph_t *graph, igraph_vector_ptr_t *vertices,
    igraph_vector_ptr_t *edges, igraph_integer_t from, igraph_vs_t to,
    const igraph_vector_t *weights, igraph_neimode_t mode,
    igraph_vector_long_t *predecessors, igraph_vector_long_t *inbound_edges) {
  /* Implementation details. This is the basic Dijkstra algorithm,
     with a binary heap. The heap is indexed, i.e. it stores not only
...
int igraph_get_shortest_path_dijkstra(
    const igraph_t *graph, igraph_vector_t *vertices, igraph_vector_t *edges,
    igraph_integer_t from, igraph_integer_t to, const igraph_vector_t *weights,
    igraph_neimode_t mode) {
  ...
  IGRAPH_CHECK(igraph_get_shortest_paths_dijkstra(
      graph, vp, ep, from, igraph_vss_1(to), weights, mode, 0, 0));
 ...
 */
int igraph_get_all_shortest_paths_dijkstra(
    const igraph_t *graph, igraph_vector_ptr_t *res, igraph_vector_t *nrgeo,
    igraph_integer_t from, igraph_vs_t to, const igraph_vector_t *weights,
    igraph_neimode_t mode) {
  ...
  */
  ...
  igraph_vector_ptr_set_item_destructor(
      &parents, (igraph_finally_func_t *)igraph_vector_destroy);
  for (i = 0; i < no_of_nodes; i++) {
  ...
  */
  ...
  if (nrgeo) {
    ...
    for (i = 1; i < n; i++) {
      ...
      for (j = 0; j < k; j++) {
        VECTOR(*nrgeo)
        [node] += VECTOR(*nrgeo)[(long int)VECTOR(*parent_vec)[j]];
      }
    }
  }
  ...
  if (res) {
    ...
    igraph_vector_ptr_set_item_destructor(
        res, (igraph_finally_func_t *)igraph_vector_destroy);
    ...
    for (i = 1; i < n; i++) {
      ...
      */
      ...
      for (j = 0; j < m; j++) {
        ...
        long int parent_path_idx =
            (long int)VECTOR(*paths_index)[parent_node] - 1;
        /*
...
int igraph_shortest_paths_johnson(const igraph_t *graph, igraph_matrix_t *res,
                                  const igraph_vs_t from, const igraph_vs_t to,
                                  const igraph_vector_t *weights) {
  ...
  if (igraph_vector_min(weights) >= 0) {
    return igraph_shortest_paths_dijkstra(graph, res, from, to, weights,
                                          IGRAPH_OUT);
  }

  if (!igraph_is_directed(graph)) {
    IGRAPH_ERROR(
        "Johnson's shortest path: undirected graph and negative weight",
        IGRAPH_EINVAL);
  }
  ...
  IGRAPH_CHECK(igraph_shortest_paths_bellman_ford(
      &newgraph, &bfres, igraph_vss_1((igraph_integer_t)no_of_nodes),
  ...
  IGRAPH_CHECK(igraph_shortest_paths_dijkstra(graph, res, from, to, &newweights,
                                              IGRAPH_OUT));
  ...
  for (i = 0; i < nr; i++, IGRAPH_VIT_NEXT(fromvit)) {
    ...
    if (igraph_vs_is_all(&to)) {
      ...
      IGRAPH_CHECK(igraph_vit_create(graph, to, &tovit));
      IGRAPH_FINALLY(igraph_vit_destroy, &tovit);
      for (j = 0, IGRAPH_VIT_RESET(tovit); j < nc;
           j++, IGRAPH_VIT_NEXT(tovit)) {
...
int igraph_unfold_tree(const igraph_t *graph, igraph_t *tree,
  ...
  for (r = 0; r < no_of_roots; r++) {
    ...
    while (!igraph_dqueue_empty(&Q)) {
      ...
      IGRAPH_CHECK(
          igraph_incident(graph, &neis, (igraph_integer_t)actnode, mode));
      n = igraph_vector_size(&neis);
      for (i = 0; i < n; i++) {
  ...
  IGRAPH_CHECK(igraph_create(tree, &edges, tree_vertex_count,
                             igraph_is_directed(graph)));
  igraph_vector_destroy(&edges);
  IGRAPH_FINALLY_CLEAN(1);
...
int igraph_is_mutual(igraph_t *graph, igraph_vector_bool_t *res,
                     igraph_es_t es) {
  ...
  IGRAPH_CHECK(igraph_lazy_adjlist_init(graph, &adjlist, IGRAPH_OUT,
  ...
  for (i = 0; !IGRAPH_EIT_END(eit); i++, IGRAPH_EIT_NEXT(eit)) {
    ...
    igraph_vector_t *neis =
        igraph_lazy_adjlist_get(&adjlist, (igraph_integer_t)to);
...
int igraph_i_avg_nearest_neighbor_degree_weighted(
    const igraph_t *graph, igraph_vs_t vids, igraph_neimode_t mode,
    igraph_neimode_t neighbor_degree_mode, igraph_vector_t *knn,
    igraph_vector_t *knnk, const igraph_vector_t *weights);
...
int igraph_i_avg_nearest_neighbor_degree_weighted(
  ...
  IGRAPH_CHECK(igraph_strength(graph, &strength, igraph_vss_all(), mode,
 ...
 * as indicated by \p mode. <code>w_uv</code> denotes the weighted adjacency
 * matrix and <code>k_v</code> is the neighbors' degree, specified by \p
...
int igraph_avg_nearest_neighbor_degree(const igraph_t *graph, igraph_vs_t vids,
                                       igraph_neimode_t mode,
  ...
  if (weights) {
    return igraph_i_avg_nearest_neighbor_degree_weighted(
        graph, vids, mode, neighbor_degree_mode, knn, knnk, weights);
  }
...
int igraph_diameter_dijkstra(const igraph_t *graph,
                             const igraph_vector_t *weights,
                             igraph_real_t *pres, igraph_integer_t *pfrom,
                             igraph_integer_t *pto, igraph_vector_t *path,
                             igraph_bool_t directed, igraph_bool_t unconn) {
  ...
  */
  ...
  if (!weights) {
    ...
    IGRAPH_CHECK(
        igraph_diameter(graph, &diameter, pfrom, pto, path, directed, unconn));
    if (pres) {
      *pres = diameter;
    }
    return IGRAPH_SUCCESS;
  ...
  if (path) {
    if (!igraph_finite(res)) {
      ...
      IGRAPH_CHECK(igraph_get_shortest_paths_dijkstra(
          graph,
          /*vertices=*/&tmpptr, /*edges=*/0, (igraph_integer_t)from,
          igraph_vss_1((igraph_integer_t)to), weights, dirmode,
          /*predecessors=*/0,
          /*inbound_edges=*/0));
      igraph_vector_ptr_destroy(&tmpptr);
      IGRAPH_FINALLY_CLEAN(1);
 ...
 * \brief Calculate a list of vertex ids sorted by degree of the corresponding
 * vertex.
 *
...
int igraph_sort_vertex_ids_by_degree(const igraph_t *graph,
                                     igraph_vector_t *outvids, igraph_vs_t vids,
...
int igraph_contract_vertices(
    igraph_t *graph, const igraph_vector_t *mapping,
  ...
  if (igraph_vector_size(mapping) != no_of_nodes) {
    IGRAPH_ERROR("Invalid mapping vector length", IGRAPH_EINVAL);
  }
  ...
  if (vattr) {
    ...
    IGRAPH_CHECK(
        igraph_i_attribute_combine_vertices(graph, &res, &merges, vertex_comb));
...
int igraph_transitive_closure_dag(const igraph_t *graph, igraph_t *closure) {
  ...
  if (!igraph_is_directed(graph)) {
    IGRAPH_ERROR("Tree transitive closure of a directed graph", IGRAPH_EINVAL);
  }
    ...
    while (!igraph_stack_empty(&path)) {
      ...
      if (node == STAR) {
        ...
        for (j = 0; j < n; j++) {
          ...
          IGRAPH_CHECK(
              igraph_vector_push_back(&new_edges, VECTOR(ancestors)[j]));
        }
      } else {
        /* Getting into a node */
        long int n, j;
        ...
        IGRAPH_CHECK(igraph_neighbors(graph, &neighbors, (igraph_integer_t)node,
                                      IGRAPH_IN));
...
int igraph_is_graphical_degree_sequence(const igraph_vector_t *out_degrees,
  ...
  if (in_degrees == 0) {
    ...
    return igraph_i_is_graphical_degree_sequence_directed(out_degrees,
                                                          in_degrees, res);
  }
}
...
int igraph_i_qsort_dual_vector_cmp_desc(void *data, const void *p1,
                                        const void *p2) {
...
static int igraph_i_is_tree_visitor(igraph_integer_t root,
                                    const igraph_adjlist_t *al,
 ...
 *        is \c IGRAPH_ALL or the graph is undirected, any vertex can be the
 * root and \p root is set to 0 (the first vertex). When \p mode is \c
 * IGRAPH_OUT or \c IGRAPH_IN, the root is set to the vertex with zero in- or
 * out-degree, respectively. \param mode For a directed graph this specifies
 * whether to test for an out-tree, an in-tree or ignore edge directions. The
 * respective possible values are: \c IGRAPH_OUT, \c IGRAPH_IN, \c IGRAPH_ALL.
 * This argument is ignored for undirected graphs. \return Error code: \c
 * IGRAPH_EINVAL: invalid mode argument.
 *
...
int igraph_is_tree(const igraph_t *graph, igraph_bool_t *res,
  ...
  /* The single-vertex graph is a tree, provided it has no edges (checked in the
   * previous if (..)) */
  if (vcount == 1) {
    *res = 1;
    if (root) {
  ...
  *res = 1; /* assume success */
  ...
  case IGRAPH_OUT: {
    ...
    IGRAPH_CHECK(igraph_degree(graph, &degree, igraph_vss_all(),
                               mode == IGRAPH_IN ? IGRAPH_OUT : IGRAPH_IN,
                               /* loops = */ 1));
