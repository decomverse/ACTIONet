...
RBERVertexPartition *
RBERVertexPartition::create(Graph *graph, vector<size_t> const &membership) {
...
double RBERVertexPartition::diff_move(size_t v, size_t new_comm) {
#ifdef DEBUG
  cerr << "double RBERVertexPartition::diff_move(" << v << ", " << new_comm
       << ")" << endl;
#endif
  ...
  if (new_comm != old_comm) {
    ...
    if (this->graph->correct_self_loops())
      possible_edge_difference_old =
          nsize * (ptrdiff_t)(2.0 * csize_old - nsize);
    else
      possible_edge_difference_old =
          nsize * (ptrdiff_t)(2.0 * csize_old - nsize - 1.0);
#ifdef DEBUG
    cerr << "\t"
         << "possible_edge_difference_old: " << possible_edge_difference_old
         << endl;
#endif
    double diff_old = w_to_old + w_from_old - self_weight -
                      this->resolution_parameter * this->graph->density() *
                          possible_edge_difference_old;
#ifdef DEBUG
    cerr << "\t"
    ...
    if (this->graph->correct_self_loops())
      possible_edge_difference_new =
          nsize * (ptrdiff_t)(2.0 * csize_new + nsize);
    else
      possible_edge_difference_new =
          nsize * (ptrdiff_t)(2.0 * csize_new + nsize - 1.0);
#ifdef DEBUG
    cerr << "\t"
         << "possible_edge_difference_new: " << possible_edge_difference_new
         << endl;
#endif
...
#ifdef DEBUG
  cerr << "exit RBERVertexPartition::diff_move(" << v << ", " << new_comm << ")"
       << endl;
...
double RBERVertexPartition::quality(double resolution_parameter) {
  ...
  for (size_t c = 0; c < this->n_communities(); c++) {
    ...
    cerr << "\t"
         ...
         << ", comm_possible_edges=" << comm_possible_edges
         << ", p=" << this->graph->density() << "." << endl;
#endif
    mod +=
        w - resolution_parameter * this->graph->density() * comm_possible_edges;
  }
#ifdef DEBUG
