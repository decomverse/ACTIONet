...
MutableVertexPartition::MutableVertexPartition(
    Graph *graph, vector<size_t> const &membership) {
...
MutableVertexPartition::create(Graph *graph, vector<size_t> const &membership) {
...
size_t MutableVertexPartition::n_communities() { return this->_n_communities; }
...
void MutableVertexPartition::init_admin() {
  ...
  // First determine number of communities (assuming they are consecutively
  // numbered
...
#ifdef DEBUG
    cerr << "\t"
         << "Add (" << v << ", " << u << ") weight " << w << " to to_comm "
         << u_comm << "." << endl;
#endif
    if (!this->graph->is_directed()) {
#ifdef DEBUG
      cerr << "\t"
           << "Add (" << u << ", " << v << ") weight " << w << " to from_comm "
           << u_comm << "." << endl;
#endif
      this->_total_weight_from_comm[u_comm] += w;
#ifdef DEBUG
      cerr << "\t"
           << "Add (" << u << ", " << v << ") weight " << w << " to to_comm "
           << v_comm << "." << endl;
#endif
  ...
  for (size_t c = 0; c < this->_n_communities; c++) {
    ...
    cerr << "\t"
         << "c=" << c << ", n_c=" << n_c
         << ", possible_edges=" << possible_edges << endl;
#endif
...
void MutableVertexPartition::renumber_communities() {
  ...
  partitions[0] = this;
  this->renumber_communities(
      MutableVertexPartition::renumber_communities(partitions));
}

vector<size_t> MutableVertexPartition::renumber_communities(
    vector<MutableVertexPartition *> partitions) {
...
void MutableVertexPartition::set_membership(vector<size_t> const &membership) {
#ifdef DEBUG
  cerr << "void MutableVertexPartition::set_membership(" << &membership << ")"
       << endl;
#endif
  for (size_t i = 0; i < this->graph->vcount(); i++) {
    this->_membership[i] = membership[i];
#ifdef DEBUG
    cerr << "Setting membership[" << i << "] = " << membership[i] << "."
         << endl;
#endif
  }
  ...
  this->init_admin();
#ifdef DEBUG
  cerr << "exit MutableVertexPartition::set_membership(" << &membership << ")"
       << endl;
#endif
}
...
size_t MutableVertexPartition::add_empty_community() {
  ...
  if (this->_n_communities > this->graph->vcount())
    throw Exception("There cannot be more communities than nodes, so there "
                    "must already be an empty community.");
...
#ifdef DEBUG
  cerr << "void MutableVertexPartition::move_node(" << v << ", " << new_comm
       << ")" << endl;
  if (new_comm >= this->n_communities())
    cerr << "ERROR: New community (" << new_comm
         << ") larger than total number of communities ("
...
#ifdef DEBUG
  cerr << "Node size: " << node_size << ", old comm: " << old_comm
       << ", new comm: " << new_comm << endl;
#endif
  // Incidentally, this is independent of whether we take into account
  // self-loops or not (i.e. whether we count as n_c^2 or as n_c(n_c - 1). Be
  // careful to do this before the adaptation of the community sizes, otherwise
  // the calculations are incorrect.
  if (new_comm != old_comm) {
    double delta_possible_edges_in_comms =
        2.0 * node_size *
        (ptrdiff_t)(this->_csize[new_comm] - this->_csize[old_comm] +
                    node_size) /
        (2.0 - this->graph->is_directed());
    _total_possible_edges_in_all_comms += delta_possible_edges_in_comms;
#ifdef DEBUG
    cerr << "Change in possible edges in all comms: "
         << delta_possible_edges_in_comms << endl;
#endif
  }
...
// Remove from old community
  ...
  // to account for nodes that have a zero size (i.e. community may not be
  // empty, but may have zero size).
...
#endif
    vector<size_t>::reverse_iterator it_comm =
        this->_empty_communities.rbegin();
    while (it_comm != this->_empty_communities.rend() && *it_comm != new_comm) {
#ifdef DEBUG
      cerr << "Empty community " << *it_comm << " != new community " << new_comm
           << endl;
#endif
      it_comm++;
    }
...
#ifdef DEBUG
  cerr << "Adding to new community " << new_comm
       << ", community size: " << this->_csize[new_comm] << endl;
#endif
...
// Switch outgoing links
  ...
  for (size_t mode_i = 0; mode_i < 2; mode_i++) {
    ...
    vector<size_t> const &neighbour_edges =
    ...
    for (size_t idx = 0; idx < degree; idx++) {
      ...
      if (mode == IGRAPH_OUT) {
        ...
        cerr << "\t"
             << "Moving link (" << v << "-" << u << ") "
             << "outgoing weight " << w << " from " << old_comm << " to "
             << new_comm << "." << endl;
#endif
      } else if (mode == IGRAPH_IN) {
        // Remove the weight from the outgoing weights of the old community
        ...
        cerr << "\t"
             << "Moving link (" << v << "-" << u << ") "
             << "incoming weight " << w << " from " << old_comm << " to "
             << new_comm << "." << endl;
#endif
      } else
      ...
      double int_weight =
          w / (this->graph->is_directed() ? 1.0 : 2.0) / (u == v ? 2.0 : 1.0);
      ...
      if (old_comm == u_comm) {
        ...
        cerr << "\t"
             << "From link (" << v << "-" << u << ") "
             << "remove internal weight " << int_weight << " from " << old_comm
             << "." << endl;
#endif
      }
      ...
      if ((new_comm == u_comm) || (u == v)) {
        ...
        cerr << "\t"
             << "From link (" << v << "-" << u << ") "
             << "add internal weight " << int_weight << " to " << new_comm
             << "." << endl;
#endif
      }
    }
  ...
  cerr << "Internal _total_weight_in_all_comms="
       << this->_total_weight_in_all_comms
       << ", calculated check_total_weight_in_all_comms="
       << check_total_weight_in_all_comms << endl;
#endif
  // Update the membership vector
  this->_membership[v] = new_comm;
#ifdef DEBUG
  cerr << "exit MutableVertexPartition::move_node(" << v << ", " << new_comm
       << ")" << endl
       << endl;
#endif
}
...
void MutableVertexPartition::from_coarse_partition(
    MutableVertexPartition *coarse_partition) {
...
void MutableVertexPartition::cache_neigh_communities(size_t v,
...
#ifdef DEBUG
  cerr << "double MutableVertexPartition::cache_neigh_communities(" << v << ", "
       << mode << ")." << endl;
#endif
...
#ifdef DEBUG
  cerr << "exit Graph::cache_neigh_communities(" << v << ", " << mode << ")."
       << endl;
#endif
}
...
set<size_t> MutableVertexPartition::get_neigh_comms(
    size_t v, igraph_neimode_t mode,
