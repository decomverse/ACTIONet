...
namespace ACTIONet {
	...
	vec NetDBSCAN(sp_mat& G, int minPts, double eps = 0.5, double alpha_val = 0.85) {
		int n = G.n_rows;
		G.diag().zeros(); // Remove self-loops
		
		// Process vertices in a particular order to give higher priority to the inner, the most well-connected vertices
		...
		for (int i = 0; i < n; i++) {			
			...
			int counts = 0;
			for(sp_mat::col_iterator col_it = G.begin_col(v); col_it != G.end_col(v); ++col_it) {
				counts ++;
				int u = col_it.row();
				if( (eps < (*col_it)) && ( (clusters[u] == UNDEFINED) || (clusters[u] == NOISE) ) )
					N.push_back(u);
			}
			...
			while(!S.empty()) {
				...
				N.clear(); // Flush previous content
				for(sp_mat::col_iterator col_it = G.begin_col(u); col_it != G.end_col(u); ++col_it) {
					int w = col_it.row();
					if( (eps < (*col_it)) && ((clusters[w] == UNDEFINED) || (clusters[w] == NOISE)) )
						N.push_back(w);
				}
				if(N.size() >= minPts) {
					while(!N.empty()) {
