...
prune_archetypes(field<mat> C_trace, field<mat> H_trace,
                 double min_specificity_z_threshold, int min_cells = 3) {
  mat C_stacked;
  mat H_stacked;
  ...
  Rprintf("Joining trace of C & H matrices (depth = %d) ... ",
  ...
  // projection on to the cell space, ish) Rprintf("Removing unreliable
  ...
  // : "archetypes"; Rprintf("Removed %d trivial %s.\n", trivial_idx.n_elem,
  // temp_str.c_str()); //fflush(stdout);
