...
sp_mat build_ACTIONet_JS_KstarNN(mat H_stacked, double density = 1.0,
  ...
  Rprintf("Building adaptive network (density = %.2f)\n", density);
  ...
  ParallelFor(0, sample_no, thread_no, [&](size_t i, size_t threadId) {
    ...
    if (result.size() != (kNN + 1)) {
      Rprintf("Unable to find %d results. Probably ef (%f) or M (%f) is too "
              "small\n",
              kNN, ef, M);
    }
  ...
  Rprintf("\tConstructing adaptive-nearest neighbor graph ... ");
  mat Delta;
  mat beta = LC * dist;
...
sp_mat build_ACTIONet_JS_KNN(mat H_stacked, double k, int thread_no = 0,
  ...
  Rprintf("Building fixed-degree network (k = %d)\n", (int)k);
  if (thread_no <= 0) {
    thread_no = SYS_THREADS_DEF; // std::thread::hardware_concurrency();
  }
  ...
  ParallelFor(0, sample_no, thread_no, [&](size_t i, size_t threadId) {
    ...
    if (result.size() != (kNN + 1)) {
      REprintf("Unable to find %d results. Probably ef (%f) or M (%f) is too "
               "small\n",
               kNN, ef, M);
    }
