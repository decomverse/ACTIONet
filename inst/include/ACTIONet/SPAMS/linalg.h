 ...
 *
 *                File linalg.h
 * \brief Contains SPAMS_Matrix, Vector classes */
...
/// Dense SPAMS_Matrix class
template<typename T> class SPAMS_Matrix;
/// Sparse SPAMS_Matrix class
template<typename T> class SpSPAMS_Matrix;
/// Dense Vector class
...
template <typename T> class Data {
   public:
      ...
      virtual void getGroup(SPAMS_Matrix<T>& data, const vector_groups& groups,
...
template <typename T> class AbstractSPAMS_MatrixB {
   public:
      virtual INTM n() const = 0;
      virtual INTM m() const = 0;
      ...
      virtual void mult(const SPAMS_Matrix<T>& B, SPAMS_Matrix<T>& C, 
            const bool transA = false, const bool transB = false,
            const T a = 1.0, const T b = 0.0) const = 0;

      virtual void mult(const SpSPAMS_Matrix<T>& B, SPAMS_Matrix<T>& C, 
            const bool transA = false, const bool transB = false,
            const T a = 1.0, const T b = 0.0) const = 0;

      /// perform C = a*B*A + b*C, possibly transposing A or B.
      virtual void multSwitch(const SPAMS_Matrix<T>& B, SPAMS_Matrix<T>& C, 
            const bool transA = false, const bool transB = false,
            const T a = 1.0, const T b = 0.0) const = 0;

      /// XtX = A'*A
      virtual void XtX(SPAMS_Matrix<T>& XtX) const = 0;

      virtual void copyRow(const INTM i, Vector<T>& x) const = 0;

      virtual void copyTo(SPAMS_Matrix<T>& copy) const = 0;
      virtual T dot(const SPAMS_Matrix<T>& x) const = 0;
      ...
      void ridgeCG(const SPAMS_Matrix<T>& b, SPAMS_Matrix<T>& x, const T lambda, const T tol, const int itermax, const int numThreads = -1) const; 
      void ridgeCG(const SPAMS_Matrix<T>& b, const SPAMS_Matrix<T>& delta, SPAMS_Matrix<T>& x, const T lambda, const T tol, const int itermax, const int numThreads = -1) const; 

      virtual ~AbstractSPAMS_MatrixB() { };
};

/// Abstract matrix class
template <typename T> class AbstractSPAMS_Matrix {
   public:
      virtual INTM n() const = 0;
      virtual INTM m() const = 0;
      /// copy X(:,i) INTMo Xi
      ...
      virtual ~AbstractSPAMS_Matrix() { };
};

/// Class SPAMS_Matrix
template<typename T> class SPAMS_Matrix : public Data<T>, public AbstractSPAMS_Matrix<T>, public AbstractSPAMS_MatrixB<T> {
	
   friend class SpSPAMS_Matrix<T>;
   public:
   ...
   SPAMS_Matrix(T* X, INTM m, INTM n);
   /// Constructor for a new m x n matrix
   SPAMS_Matrix(INTM m, INTM n);
   /// Empty constructor
   SPAMS_Matrix();

   /// Destructor
   virtual ~SPAMS_Matrix();
   ...
   /// Reference the column i to i+n into the SPAMS_Matrix mat
   inline void refSubMat(INTM i, INTM n, SPAMS_Matrix<T>& mat) const;
   /// extract a sub-matrix of a symmetric matrix
   inline void subSPAMS_MatrixSym(const Vector<INTM>& indices, 
         SPAMS_Matrix<T>& subSPAMS_Matrix) const;
   ...
   inline void copy(const SPAMS_Matrix<T>& mat);
   /// make a copy of the matrix mat in the current matrix
   inline void copyTo(SPAMS_Matrix<T>& mat) const { mat.copy(*this); };
   /// make a copy of the matrix mat in the current matrix
   inline void copyRef(const SPAMS_Matrix<T>& mat);
   ...
   /// trans
   inline void transpose(SPAMS_Matrix<T>& trans);
   /// A <- -A
   inline void neg();
   ...
   inline void svd(SPAMS_Matrix<T>& U, Vector<T>& S, SPAMS_Matrix<T>&V) const;
   ...
   inline void mult(const SpVector<T>& x, Vector<T>& b, 
         const T alpha = 1.0, const T beta = 0.0) const;
   /// perform C = a*A*B + b*C, possibly transposing A or B.
   inline void mult(const SPAMS_Matrix<T>& B, SPAMS_Matrix<T>& C, 
         const bool transA = false, const bool transB = false,
         const T a = 1.0, const T b = 0.0) const;
   /// perform C = a*B*A + b*C, possibly transposing A or B.
   inline void multSwitch(const SPAMS_Matrix<T>& B, SPAMS_Matrix<T>& C, 
         const bool transA = false, const bool transB = false,
         const T a = 1.0, const T b = 0.0) const;
   /// perform C = A*B, when B is sparse
   inline void mult(const SpSPAMS_Matrix<T>& B, SPAMS_Matrix<T>& C, const bool transA = false,
   ...
   inline void mult_elementWise(const SPAMS_Matrix<T>& B, SPAMS_Matrix<T>& C) const;
   inline void div_elementWise(const SPAMS_Matrix<T>& B, SPAMS_Matrix<T>& C) const;
   /// XtX = A'*A
   inline void XtX(SPAMS_Matrix<T>& XtX) const;
   /// XXt = A*A'
   inline void XXt(SPAMS_Matrix<T>& XXt) const;
   /// XXt = A*A' where A is an upper triangular matrix
   inline void upperTriXXt(SPAMS_Matrix<T>& XXt, 
         const INTM L) const;
   ...
   inline void add(const SPAMS_Matrix<T>& mat, const T alpha = 1.0);
   /// add alpha to the current matrix
   inline void add(const T alpha);
   /// add alpha*mat to the current matrix
   inline T dot(const SPAMS_Matrix<T>& mat) const;
   /// substract the matrix mat to the current matrix
   inline void sub(const SPAMS_Matrix<T>& mat);
   ...
   inline void sparseProject(SPAMS_Matrix<T>& out, const T thrs,   const int mode = 1, const T lambda1 = 0,
   ...
   inline void toSparse(SpSPAMS_Matrix<T>& matrix) const;
   /// make a sparse copy of the current matrix
   inline void toSparseTrans(SpSPAMS_Matrix<T>& matrixTrans);
   /// make a reference of the matrix to a vector vec 
   inline void toVect(Vector<T>& vec) const;
   /// Accessor
   inline INTM V() const { return 1;};
   /// merge two dictionaries
   inline void merge(const SPAMS_Matrix<T>& B, SPAMS_Matrix<T>& C) const;
   /// extract the rows of a matrix corresponding to a binary mask
   inline void copyMask(SPAMS_Matrix<T>& out, Vector<bool>& mask) const;
   ...
   protected:
   /// Forbid lazy copies
   explicit SPAMS_Matrix<T>(const SPAMS_Matrix<T>& matrix);
   /// Forbid lazy copies
   SPAMS_Matrix<T>& operator=(const SPAMS_Matrix<T>& matrix);
...
/// Sparse SPAMS_Matrix class, CSC format
template<typename T> class SpSPAMS_Matrix : public Data<T>, public AbstractSPAMS_MatrixB<T> {
   friend class SPAMS_Matrix<T>;
   friend class SpVector<T>;
   public:
   /// Constructor, CSC format, existing data
   SpSPAMS_Matrix(T* v, INTM* r, INTM* pB, INTM* pE, INTM m, INTM n, INTM nzmax);
   /// Constructor, new m x n matrix, with at most nzmax non-zeros values
   SpSPAMS_Matrix(INTM m, INTM n, INTM nzmax);
   /// Empty constructor
   SpSPAMS_Matrix();
   ...
   void getGroup(SPAMS_Matrix<T>& data, const vector_groups& groups,
         const INTM i) const ;
   ...
   inline void add_direct(const SpSPAMS_Matrix<T>& mat, const T a);
   inline void copy_direct(const SpSPAMS_Matrix<T>& mat);
   inline T dot_direct(const SpSPAMS_Matrix<T>& mat) const;
   ...
   /// aat <- A*A'
   inline void AAt(SPAMS_Matrix<T>& aat) const;
   /// aat <- A(:,indices)*A(:,indices)'
   inline void AAt(SPAMS_Matrix<T>& aat, const Vector<INTM>& indices) const;
   /// aat <- sum_i w_i A(:,i)*A(:,i)'
   inline void wAAt(const Vector<T>& w, SPAMS_Matrix<T>& aat) const;
   /// XAt <- X*A'
   inline void XAt(const SPAMS_Matrix<T>& X, SPAMS_Matrix<T>& XAt) const;
   /// XAt <- X(:,indices)*A(:,indices)'
   inline void XAt(const SPAMS_Matrix<T>& X, SPAMS_Matrix<T>& XAt, 
         const Vector<INTM>& indices) const;
   /// XAt <- sum_i w_i X(:,i)*A(:,i)'
   inline void wXAt( const Vector<T>& w, const SPAMS_Matrix<T>& X, 
         SPAMS_Matrix<T>& XAt, const int numthreads=-1) const;
   inline void XtX(SPAMS_Matrix<T>& XtX) const;
   ...
   inline void mult(const SpSPAMS_Matrix<T>& B, SPAMS_Matrix<T>& C, const bool transA = false,
         ...
         const T b = 0.0) const;
   /// make a copy of the matrix mat in the current matrix
   inline void copyTo(SPAMS_Matrix<T>& mat) const { this->toFull(mat); };
   /// dot product;
   inline T dot(const SPAMS_Matrix<T>& x) const;
   inline void copyRow(const INTM i, Vector<T>& x) const;
   inline void sum_cols(Vector<T>& sum) const;
   inline void copy(const SpSPAMS_Matrix<T>& mat);

   /// Conversions
   /// copy the sparse matrix into a dense matrix
   inline void toFull(SPAMS_Matrix<T>& matrix) const;
   /// copy the sparse matrix into a dense transposed matrix
   inline void toFullTrans(SPAMS_Matrix<T>& matrix) const;

   /// use the data from v, r for _v, _r
   inline void convert(const SPAMS_Matrix<T>&v, const SPAMS_Matrix<INTM>& r,
         const INTM K);
   /// use the data from v, r for _v, _r
   inline void convert2(const SPAMS_Matrix<T>&v, const Vector<INTM>& r,
         const INTM K);
   /// returns the l2 norms ^2 of the columns
   inline void norm_2sq_cols(Vector<T>& norms) const;
   ...
   explicit SpSPAMS_Matrix(const SpSPAMS_Matrix<T>& matrix);
   SpSPAMS_Matrix<T>& operator=(const SpSPAMS_Matrix<T>& matrix);
...
template <typename T> class SpVector {
   ...
   void inline toSpSPAMS_Matrix(SpSPAMS_Matrix<T>& out,
...
template<typename T> class ProdSPAMS_Matrix : public AbstractSPAMS_Matrix<T> {
   public:
      ProdSPAMS_Matrix();
      /// Constructor. SPAMS_Matrix D'*D is represented
      ProdSPAMS_Matrix(const SPAMS_Matrix<T>& D, const bool high_memory = true);
      /// Constructor. SPAMS_Matrix D'*X is represented
      ProdSPAMS_Matrix(const SPAMS_Matrix<T>& D, const SPAMS_Matrix<T>& X, const bool high_memory = true);
      /// Constructor, D'*X is represented, with optional transpositions
      /*ProdSPAMS_Matrix(const SpSPAMS_Matrix<T>& D, const SPAMS_Matrix<T>& X,
        const bool transD = false, const bool transX = false);*/

      /// Destructor
      ~ProdSPAMS_Matrix() { delete(_DtX);} ;

      /// set_matrices
      inline void setMatrices(const SPAMS_Matrix<T>& D, const bool high_memory=true);
      inline void setMatrices(const SPAMS_Matrix<T>& D, 
            const SPAMS_Matrix<T>& X, const bool high_memory=true);
      /// compute DtX(:,i)
   ...
   private:
      ...
      SPAMS_Matrix<T>* _DtX;
      const SPAMS_Matrix<T>* _X;
      const SPAMS_Matrix<T>* _D;
      bool _high_memory;
      INTM _n;
      INTM _m;
      T _addDiag;
...
/* ************************************
 * Implementation of the class SPAMS_Matrix 
 * ************************************/

/// Constructor with existing data X of an m x n matrix
template <typename T> SPAMS_Matrix<T>::SPAMS_Matrix(T* X, INTM m, INTM n) :
   _externAlloc(true), _X(X), _m(m), _n(n) {  };


/// Constructor for a new m x n matrix
template <typename T> SPAMS_Matrix<T>::SPAMS_Matrix(INTM m, INTM n) :
...
/// Empty constructor
template <typename T> SPAMS_Matrix<T>::SPAMS_Matrix() :
...
/// Return a modifiable reference to X(i,j)
template <typename T> inline T& SPAMS_Matrix<T>::operator()(const INTM i, const INTM j) {
   return _X[j*_m+i];
};

/// Return the value X(i,j)
template <typename T> inline T SPAMS_Matrix<T>::operator()(const INTM i, const INTM j) const {
   return _X[j*_m+i];
};

/// Print the matrix to std::cout
template <typename T> inline void SPAMS_Matrix<T>::print(const string& name) const {
...
template <typename T> inline void SPAMS_Matrix<T>::copyCol(const INTM i, Vector<T>& x) const {
   assert(i >= 0 && i<_n);
   x.resize(_m);
...
/// Copy the column i INTMo x
template <typename T> inline void SPAMS_Matrix<T>::copyRow(const INTM i, Vector<T>& x) const {
   assert(i >= 0 && i<_m);
   x.resize(_n);
...
/// Copy the column i INTMo x
template <typename T> inline void SPAMS_Matrix<T>::extract_rawCol(const INTM i, T* x) const {
   assert(i >= 0 && i<_n);
   cblas_copy<T>(_m,_X+i*_m,1,x,1);
};

/// Copy the column i INTMo x
template <typename T> inline void SPAMS_Matrix<T>::add_rawCol(const INTM i, T* x, const T a) const {
   assert(i >= 0 && i<_n);
   cblas_axpy<T>(_m,a,_X+i*_m,1,x,1);
};

/// Copy the column i INTMo x
template <typename T> inline void SPAMS_Matrix<T>::getData(Vector<T>& x, const INTM i) const {
   this->copyCol(i,x);
};

template <typename T> inline void SPAMS_Matrix<T>::getGroup(SPAMS_Matrix<T>& data, 
...
/// Reference the column i into the vector x
template <typename T> inline void SPAMS_Matrix<T>::refCol(INTM i, Vector<T>& x) const {
   assert(i >= 0 && i<_n);
   x.clear();
   x._X=_X+i*_m;
   x._n=_m;
...
/// Reference the column i to i+n INTMo the SPAMS_Matrix mat
template <typename T> inline void SPAMS_Matrix<T>::refSubMat(INTM i, INTM n, SPAMS_Matrix<T>& mat) const {
   mat.setData(_X+i*_m,_m,n);
}

/// Check wether the columns of the matrix are normalized or not
template <typename T> inline bool SPAMS_Matrix<T>::isNormalized() const {
   for (INTM i = 0; i<_n; ++i) {
...
/// clean a dictionary matrix
template <typename T>
inline void SPAMS_Matrix<T>::clean() {
   this->normalize();
   SPAMS_Matrix<T> G;
   this->XtX(G);
   T* prG = G._X;
...
/// return the 1D-index of the value of greatest magnitude
template <typename T> inline INTM SPAMS_Matrix<T>::fmax() const {
   return cblas_iamax<T>(_n*_m,_X,1);
};

/// return the value of greatest magnitude
template <typename T> inline T SPAMS_Matrix<T>::fmaxval() const {
   return _X[cblas_iamax<T>(_n*_m,_X,1)];
};


/// return the 1D-index of the value of lowest magnitude
template <typename T> inline INTM SPAMS_Matrix<T>::fmin() const {
   return cblas_iamin<T>(_n*_m,_X,1);
};

/// extract a sub-matrix of a symmetric matrix
template <typename T> inline void SPAMS_Matrix<T>::subSPAMS_MatrixSym(
      const Vector<INTM>& indices, SPAMS_Matrix<T>& subSPAMS_Matrix) const {
   INTM L = indices.n();
   subSPAMS_Matrix.resize(L,L);
   T* out = subSPAMS_Matrix._X;
   INTM* rawInd = indices.rawX();
   for (INTM i = 0; i<L; ++i)
      for (INTM j = 0; j<=i; ++j)
         out[i*L+j]=_X[rawInd[i]*_n+rawInd[j]];
   subSPAMS_Matrix.fillSymmetric();
};

/// Resize the matrix
template <typename T> inline void SPAMS_Matrix<T>::resize(INTM m, INTM n, const bool set_zeros) {
   if (_n==n && _m==m) return;
   clear();
   _n=n;
   _m=m;
...
/// Change the data in the matrix
template <typename T> inline void SPAMS_Matrix<T>::setData(T* X, INTM m, INTM n) {
   clear();
   _X=X;
   _m=m;
   _n=n;
...
/// Set all the values to zero
template <typename T> inline void SPAMS_Matrix<T>::setZeros() {
   memset(_X,0,_n*_m*sizeof(T));
};

/// Set all the values to a scalar
template <typename T> inline void SPAMS_Matrix<T>::set(const T a) {
   for (INTM i = 0; i<_n*_m; ++i) _X[i]=a;
};

/// Clear the matrix
template <typename T> inline void SPAMS_Matrix<T>::clear() {
...
/// Put white Gaussian noise in the matrix 
template <typename T> inline void SPAMS_Matrix<T>::setAleat() {
   for (INTM i = 0; i<_n*_m; ++i) _X[i]=normalDistrib<T>();
};

/// set the matrix to the identity
template <typename T> inline void SPAMS_Matrix<T>::eye() {
   this->setZeros();
   for (INTM i = 0; i<MIN(_n,_m); ++i) _X[i*_m+i] = T(1.0);
};

/// Normalize all columns to unit l2 norm
template <typename T> inline void SPAMS_Matrix<T>::normalize() {
...
/// Normalize all columns which l2 norm is greater than one.
template <typename T> inline void SPAMS_Matrix<T>::normalize2() {
   for (INTM i = 0; i<_n; ++i) {
      T norm=cblas_nrm2<T>(_m,_X+_m*i,1);
      if (norm > 1.0) {
         T invNorm=1.0/norm;
...
/// center the matrix
template <typename T> inline void SPAMS_Matrix<T>::center() {
   for (INTM i = 0; i<_n; ++i) {
      Vector<T> col;
      this->refCol(i,col);
      T sum = col.sum();
...
/// center the matrix
template <typename T> inline void SPAMS_Matrix<T>::center_rows() {
...
template <typename T> inline void SPAMS_Matrix<T>::center(Vector<T>& centers) {
   centers.resize(_n);
   for (INTM i = 0; i<_n; ++i) {
      Vector<T> col;
      this->refCol(i,col);
...
/// scale the matrix by the a
template <typename T> inline void SPAMS_Matrix<T>::scal(const T a) {
   cblas_scal<T>(_n*_m,a,_X,1);
};

/// make a copy of the matrix mat in the current matrix
template <typename T> inline void SPAMS_Matrix<T>::copy(const SPAMS_Matrix<T>& mat) {
...
/// make a copy of the matrix mat in the current matrix
template <typename T> inline void SPAMS_Matrix<T>::copyRef(const SPAMS_Matrix<T>& mat) {
   this->setData(mat.rawX(),mat.m(),mat.n());
};

/// make the matrix symmetric by copying the upper-right part
/// INTMo the lower-left part
template <typename T> inline void SPAMS_Matrix<T>::fillSymmetric() {
   for (INTM i = 0; i<_n; ++i) {
      for (INTM j =0; j<i; ++j) {
         _X[j*_m+i]=_X[i*_m+j];
      }
...
template <typename T> inline void SPAMS_Matrix<T>::fillSymmetric2() {
   for (INTM i = 0; i<_n; ++i) {
      for (INTM j =0; j<i; ++j) {
         _X[i*_m+j]=_X[j*_m+i];
      }
...
template <typename T> inline void SPAMS_Matrix<T>::whiten(const INTM V) {
...
template <typename T> inline void SPAMS_Matrix<T>::whiten(Vector<T>& mean, const bool pattern) {
   mean.setZeros();
   if (pattern) {
...
template <typename T> inline void SPAMS_Matrix<T>::whiten(Vector<T>& mean, const
      Vector<T>& mask) {
   const INTM V = mean.n();
   const INTM sizePatch=_m/V;
   mean.setZeros();
...
template <typename T> inline void SPAMS_Matrix<T>::unwhiten(Vector<T>& mean, const bool pattern) {
   if (pattern) {
      const INTM n =static_cast<INTM>(sqrt(static_cast<T>(_m)));
      for (INTM i = 0; i<_n; ++i) {
         INTM offsetx=0;
...
/// Transpose the current matrix and put the result in the matrix
/// trans
template <typename T> inline void SPAMS_Matrix<T>::transpose(SPAMS_Matrix<T>& trans) {
   trans.resize(_n,_m);
   T* out = trans._X;
   for (INTM i = 0; i<_n; ++i)
      for (INTM j = 0; j<_m; ++j)
...
/// A <- -A
template <typename T> inline void SPAMS_Matrix<T>::neg() {
   for (INTM i = 0; i<_n*_m; ++i) _X[i]=-_X[i];
};

template <typename T> inline void SPAMS_Matrix<T>::incrDiag() {
   for (INTM i = 0; i<MIN(_n,_m); ++i) ++_X[i*_m+i];
};

template <typename T> inline void SPAMS_Matrix<T>::addDiag(
      const Vector<T>& diag) {
   T* d= diag.rawX();
...
template <typename T> inline void SPAMS_Matrix<T>::addToCols(
      const Vector<T>& cent) {
   Vector<T> col;
   for (INTM i = 0; i<_n; ++i) {
      this->refCol(i,col);      
...
template <typename T> inline void SPAMS_Matrix<T>::addVecToCols(
...
/// perform a rank one approximation uv' using the power method
/// u0 is an initial guess for u (can be empty).
template <typename T> inline void SPAMS_Matrix<T>::svdRankOne(const Vector<T>& u0,
      Vector<T>& u, Vector<T>& v) const {
   int i;
...
template <typename T> inline void SPAMS_Matrix<T>::singularValues(Vector<T>& u) const {
   u.resize(MIN(_m,_n));
   if (_m > 10*_n) {
      SPAMS_Matrix<T> XtX;
      this->XtX(XtX);
      ...
      T* vu = NULL;
      T* vv = NULL;
      SPAMS_Matrix<T> copyX;
      copyX.copy(*this);
      gesvd<T>(no,no,_m,_n,copyX._X,_m,u.rawX(),vu,1,vv,1);
   }
};

template <typename T> inline void SPAMS_Matrix<T>::svd(SPAMS_Matrix<T>& U, Vector<T>& S, SPAMS_Matrix<T>&V) const {
   const INTM num_eig=MIN(_m,_n);
   S.resize(num_eig);
   U.resize(_m,num_eig);
   V.resize(num_eig,_n);
   if (_m > 10*_n) {
      SPAMS_Matrix<T> Vt(_n,_n);
      this->XtX(Vt);
...
/// when the current matrix is symmetric. u0 is the initial guess.
/// using two iterations of the power method
template <typename T> inline void SPAMS_Matrix<T>::eigLargestSymApprox(
...
/// returns the eigenvalue. u0 is an initial guess for the 
/// eigenvector.
template <typename T> inline T SPAMS_Matrix<T>::eigLargestMagnSym(
      const Vector<T>& u0, Vector<T>& u) const {
...
/// returns the value of the eigenvalue with the largest magnitude
/// using the power iteration.
template <typename T> inline T SPAMS_Matrix<T>::eigLargestMagnSym() const {
   const int max_iter=1000;
   const T eps=10e-6;
   Vector<T> u(_m);
   u.setAleat();
...
/// inverse the matrix when it is symmetric
template <typename T> inline void SPAMS_Matrix<T>::invSym() {
   sytri<T>(upper,_n,_X,_n);
   this->fillSymmetric();
};

/// perform b = alpha*A'x + beta*b
template <typename T> inline void SPAMS_Matrix<T>::multTrans(const Vector<T>& x, 
...
/// perform b = A'x, when x is sparse
template <typename T> inline void SPAMS_Matrix<T>::multTrans(const SpVector<T>& x, 
...
template <typename T> inline void SPAMS_Matrix<T>::multTrans(
...
/// perform b = alpha*A*x+beta*b
template <typename T> inline void SPAMS_Matrix<T>::mult(const Vector<T>& x, 
      Vector<T>& b, const T a, const T c) const {
...
/// perform b = alpha*A*x + beta*b, when x is sparse
template <typename T> inline void SPAMS_Matrix<T>::mult(const SpVector<T>& x, 
...
/// perform C = a*A*B + b*C, possibly transposing A or B.
template <typename T> inline void SPAMS_Matrix<T>::mult(const SPAMS_Matrix<T>& B, 
      SPAMS_Matrix<T>& C, const bool transA, const bool transB,
...
/// perform C = a*B*A + b*C, possibly transposing A or B.
template <typename T>
inline void SPAMS_Matrix<T>::multSwitch(const SPAMS_Matrix<T>& B, SPAMS_Matrix<T>& C, 
...
/// perform C = A*B, when B is sparse
template <typename T>
inline void SPAMS_Matrix<T>::mult(const SpSPAMS_Matrix<T>& B, SPAMS_Matrix<T>& C,
      const bool transA, const bool transB,
      const T a, const T b) const {
   if (transA) {
      if (transB) {
...
/// mult by a diagonal matrix on the left
template <typename T>
   inline void SPAMS_Matrix<T>::multDiagLeft(const Vector<T>& diag) {
      if (diag.n() != _m)
         return;
      T* d = diag.rawX();
      for (INTM i = 0; i< _n; ++i) {
...
/// mult by a diagonal matrix on the right
template <typename T> inline void SPAMS_Matrix<T>::multDiagRight(
      const Vector<T>& diag) {
   if (diag.n() != _n)
      return;
   T* d = diag.rawX();
...
/// C = A .* B, elementwise multiplication
template <typename T> inline void SPAMS_Matrix<T>::mult_elementWise(
      const SPAMS_Matrix<T>& B, SPAMS_Matrix<T>& C) const {
...
/// C = A .* B, elementwise multiplication
template <typename T> inline void SPAMS_Matrix<T>::div_elementWise(
      const SPAMS_Matrix<T>& B, SPAMS_Matrix<T>& C) const {
   assert(_n == B._n && _m == B._m);
   C.resize(_m,_n);
...
/// XtX = A'*A
template <typename T> inline void SPAMS_Matrix<T>::XtX(SPAMS_Matrix<T>& xtx) const {
   xtx.resize(_n,_n);
...
/// XXt = A*At
template <typename T> inline void SPAMS_Matrix<T>::XXt(SPAMS_Matrix<T>& xxt) const {
   xxt.resize(_m,_m);
...
/// XXt = A*A' where A is an upper triangular matrix
template <typename T> inline void SPAMS_Matrix<T>::upperTriXXt(SPAMS_Matrix<T>& XXt, const INTM L) const {
   XXt.resize(L,L);
   for (INTM i = 0; i<L; ++i) {
...
/// extract the diagonal
template <typename T> inline void SPAMS_Matrix<T>::diag(Vector<T>& dv) const {
...
/// set the diagonal
template <typename T> inline void SPAMS_Matrix<T>::setDiag(const Vector<T>& dv) {
   INTM size_diag=MIN(_n,_m);
   T* const d = dv.rawX();
   for (INTM i = 0; i<size_diag; ++i)
      _X[i*_m+i]=d[i];
...
/// set the diagonal
template <typename T> inline void SPAMS_Matrix<T>::setDiag(const T val) {
   INTM size_diag=MIN(_n,_m);
   for (INTM i = 0; i<size_diag; ++i)
      _X[i*_m+i]=val;
};
...
/// each element of the matrix is replaced by its exponential
template <typename T> inline void SPAMS_Matrix<T>::exp() {
   vExp<T>(_n*_m,_X,_X);
};
template <typename T> inline void SPAMS_Matrix<T>::Sqrt() {
   vSqrt<T>(_n*_m,_X,_X);
};

template <typename T> inline void SPAMS_Matrix<T>::Invsqrt() {
   vInvSqrt<T>(_n*_m,_X,_X);
};
/// return vec1'*A*vec2, where vec2 is sparse
template <typename T> inline T SPAMS_Matrix<T>::quad(
...
template <typename T> inline void SPAMS_Matrix<T>::quad_mult(const Vector<T>& vec1,
...
/// add alpha*mat to the current matrix
template <typename T> inline void SPAMS_Matrix<T>::add(const SPAMS_Matrix<T>& mat, const T alpha) {
   assert(mat._m == _m && mat._n == _n);
   cblas_axpy<T>(_n*_m,alpha,mat._X,1,_X,1);
};

/// add alpha*mat to the current matrix
template <typename T> inline T SPAMS_Matrix<T>::dot(const SPAMS_Matrix<T>& mat) const {
   assert(mat._m == _m && mat._n == _n);
...
/// add alpha to the current matrix
template <typename T> inline void SPAMS_Matrix<T>::add(const T alpha) {
   for (INTM i = 0; i<_n*_m; ++i) _X[i]+=alpha;
};

/// substract the matrix mat to the current matrix
template <typename T> inline void SPAMS_Matrix<T>::sub(const SPAMS_Matrix<T>& mat) {
   vSub<T>(_n*_m,_X,mat._X,_X);
};

/// compute the sum of the magnitude of the matrix values
template <typename T> inline T SPAMS_Matrix<T>::asum() const {
   return cblas_asum<T>(_n*_m,_X,1);
};

/// returns the trace of the matrix
template <typename T> inline T SPAMS_Matrix<T>::trace() const {
   T sum=T();
   INTM m = MIN(_n,_m);
   for (INTM i = 0; i<m; ++i) 
      sum += _X[i*_m+i];
...
/// return ||A||_F
template <typename T> inline T SPAMS_Matrix<T>::normF() const {
   return cblas_nrm2<T>(_n*_m,_X,1);
};

template <typename T> inline T SPAMS_Matrix<T>::mean() const {
   Vector<T> vec;
   this->toVect(vec);
   return vec.mean();
};
...
/// return ||A||_F^2
template <typename T> inline T SPAMS_Matrix<T>::normFsq() const {
   return cblas_dot<T>(_n*_m,_X,1,_X,1);
};

/// return ||At||_{inf,2}
template <typename T> inline T SPAMS_Matrix<T>::norm_inf_2_col() const {
   Vector<T> col;
   T max = -1.0;
   for (INTM i = 0; i<_n; ++i) {
      refCol(i,col);
...
/// return ||At||_{1,2}
template <typename T> inline T SPAMS_Matrix<T>::norm_1_2_col() const {
   Vector<T> col;
   T sum = 0.0;
   for (INTM i = 0; i<_n; ++i) {
      refCol(i,col);
...
/// returns the l2 norms of the columns
template <typename T> inline void SPAMS_Matrix<T>::norm_2_rows(
...
/// returns the l2 norms of the columns
template <typename T> inline void SPAMS_Matrix<T>::norm_2sq_rows(
      Vector<T>& norms) const {
   norms.resize(_m);
   norms.setZeros();
   for (INTM i = 0; i<_n; ++i) 
...
/// returns the l2 norms of the columns
template <typename T> inline void SPAMS_Matrix<T>::norm_2_cols(
      Vector<T>& norms) const {
   norms.resize(_n);
   Vector<T> col;
   for (INTM i = 0; i<_n; ++i) {
...
/// returns the linf norms of the columns
template <typename T> inline void SPAMS_Matrix<T>::norm_inf_cols(Vector<T>& norms) const {
   norms.resize(_n);
   Vector<T> col;
   for (INTM i = 0; i<_n; ++i) {
      refCol(i,col);
...
/// returns the linf norms of the columns
template <typename T> inline void SPAMS_Matrix<T>::norm_inf_rows(Vector<T>& norms) const {
   norms.resize(_m);
   norms.setZeros();
   for (INTM i = 0; i<_n; ++i) 
      for (INTM j = 0; j<_m; ++j) 
...
/// returns the linf norms of the columns
template <typename T> inline void SPAMS_Matrix<T>::norm_l1_rows(Vector<T>& norms) const {
   norms.resize(_m);
   norms.setZeros();
   for (INTM i = 0; i<_n; ++i) 
      for (INTM j = 0; j<_m; ++j) 
...
/// returns the l2 norms of the columns
template <typename T> inline void SPAMS_Matrix<T>::norm_2sq_cols(
      Vector<T>& norms) const {
   norms.resize(_n);
   Vector<T> col;
   for (INTM i = 0; i<_n; ++i) {
...
template <typename T> 
inline void SPAMS_Matrix<T>::sum_cols(Vector<T>& sum) const {
   sum.resize(_m);
   sum.setZeros();
   Vector<T> tmp;
   for (INTM i = 0; i<_n; ++i) {
...
/// Compute the mean of the columns
template <typename T> inline void SPAMS_Matrix<T>::meanCol(Vector<T>& mean) const {
   Vector<T> ones(_n);
   ones.set(T(1.0/_n));
...
/// Compute the mean of the rows
template <typename T> inline void SPAMS_Matrix<T>::meanRow(Vector<T>& mean) const {
   Vector<T> ones(_m);
   ones.set(T(1.0/_m));
...
/// fill the matrix with the row given
template <typename T> inline void SPAMS_Matrix<T>::fillRow(const Vector<T>& row) {
   for (INTM i = 0; i<_n; ++i) {
      T val = row[i];
      for (INTM j = 0; j<_m; ++j) {
         _X[i*_m+j]=val;
...
/// fill the matrix with the row given
template <typename T> inline void SPAMS_Matrix<T>::extractRow(const INTM j,
      Vector<T>& row) const {
   row.resize(_n);
   for (INTM i = 0; i<_n; ++i) {
      row[i]=_X[i*_m+j];
...
/// fill the matrix with the row given
template <typename T> inline void SPAMS_Matrix<T>::setRow(const INTM j,
      const Vector<T>& row) {
   for (INTM i = 0; i<_n; ++i) {
      _X[i*_m+j]=row[i];
   }
...
/// fill the matrix with the row given
template <typename T> inline void SPAMS_Matrix<T>::addRow(const INTM j,
...
/// perform soft-thresholding of the matrix, with the threshold nu
template <typename T> inline void SPAMS_Matrix<T>::softThrshold(const T nu) {
   Vector<T> vec;
   toVect(vec);
   vec.softThrshold(nu);
};
...
/// perform soft-thresholding of the matrix, with the threshold nu
template <typename T> inline void SPAMS_Matrix<T>::hardThrshold(const T nu) {
   Vector<T> vec;
   toVect(vec);
   vec.hardThrshold(nu);
};
...
template <typename T> inline void SPAMS_Matrix<T>::thrsmax(const T nu) {
   Vector<T> vec;
   toVect(vec);
   vec.thrsmax(nu);
};
...
/// perform thresholding of the matrix, with the threshold nu
template <typename T> inline void SPAMS_Matrix<T>::thrsmin(const T nu) {
   Vector<T> vec;
   toVect(vec);
   vec.thrsmin(nu);
};
...
/// perform soft-thresholding of the matrix, with the threshold nu
template <typename T> inline void SPAMS_Matrix<T>::inv_elem() {
   Vector<T> vec;
   toVect(vec);
   vec.inv();
};
...
/// perform soft-thresholding of the matrix, with the threshold nu
template <typename T> inline void SPAMS_Matrix<T>::blockThrshold(const T nu,
...
template <typename T> inline void SPAMS_Matrix<T>::sparseProject(SPAMS_Matrix<T>& Y, 
...
/// perform soft-thresholding of the matrix, with the threshold nu
template <typename T> inline void SPAMS_Matrix<T>::thrsPos() {
   Vector<T> vec;
   toVect(vec);
   vec.thrsPos();
};
...
/// perform A <- A + alpha*vec1*vec2'
template <typename T> inline void SPAMS_Matrix<T>::rank1Update(
...
template <typename T>
inline void SPAMS_Matrix<T>::rank1Update_mult(const Vector<T>& vec1, 
...
template <typename T> inline void SPAMS_Matrix<T>::conjugateGradient(
...
template <typename T> inline void SPAMS_Matrix<T>::drop(char* fileName) const {
   std::ofstream f;
   f.precision(12);
   f.flags(std::ios_base::scientific);
   f.open(fileName, ofstream::trunc);
   std::cout << "SPAMS_Matrix written in " << fileName << std::endl;
   for (INTM i = 0; i<_n; ++i) {
      for (INTM j = 0; j<_m; ++j) 
         f << _X[i*_m+j] << " ";
      f << std::endl;
...
/// compute a Nadaraya Watson estimator
template <typename T> inline void SPAMS_Matrix<T>::NadarayaWatson(
...
/// make a sparse copy of the current matrix
template <typename T> inline void SPAMS_Matrix<T>::toSparse(SpSPAMS_Matrix<T>& out) const {
   out.clear();
   INTM count=0;
   INTM* pB;
#pragma omp critical
...
/// make a sparse copy of the current matrix
template <typename T> inline void SPAMS_Matrix<T>::toSparseTrans(
      SpSPAMS_Matrix<T>& out) {
   out.clear();
   INTM count=0;
   INTM* pB;
#pragma omp critical
...
/// make a reference of the matrix to a vector vec 
template <typename T> inline void SPAMS_Matrix<T>::toVect(
      Vector<T>& vec) const {
   vec.clear();
...
/// merge two dictionaries
template <typename T> inline void SPAMS_Matrix<T>::merge(const SPAMS_Matrix<T>& B,
      SPAMS_Matrix<T>& C) const {
   const INTM K =_n; 
   SPAMS_Matrix<T> G;
...
template <typename T>
inline void SPAMS_Matrix<T>::copyMask(SPAMS_Matrix<T>& out, Vector<bool>& mask) const {
   out.resize(_m,_n);
   INTM count=0;
...
/* ****************************
 * Implementation of SpSPAMS_Matrix 
 * ****************************/


/// Constructor, CSC format, existing data
template <typename T> SpSPAMS_Matrix<T>::SpSPAMS_Matrix(T* v, INTM* r, INTM* pB, INTM* pE,
      INTM m, INTM n, INTM nzmax) :
   _externAlloc(true), _v(v), _r(r), _pB(pB), _pE(pE), _m(m), _n(n), _nzmax(nzmax)
{ };

/// Constructor, new m x n matrix, with at most nzmax non-zeros values
template <typename T> SpSPAMS_Matrix<T>::SpSPAMS_Matrix(INTM m, INTM n, INTM nzmax) :
...
/// Empty constructor
template <typename T> SpSPAMS_Matrix<T>::SpSPAMS_Matrix() :
   _externAlloc(true), _v(NULL), _r(NULL), _pB(NULL), _pE(NULL),
   _m(0),_n(0),_nzmax(0) { };


template <typename T>
inline void SpSPAMS_Matrix<T>::copy(const SpSPAMS_Matrix<T>& mat) {
...
/// reference the column i INTMo vec
template <typename T> inline void SpSPAMS_Matrix<T>::refCol(INTM i, 
      SpVector<T>& vec) const {
...
/// print the sparse matrix
template<typename T> inline void SpSPAMS_Matrix<T>::print(const string& name) const {
   cerr << name << endl;
...
template<typename T>
inline T SpSPAMS_Matrix<T>::operator[](const INTM index) const {
...
template<typename T>
void SpSPAMS_Matrix<T>::getData(Vector<T>& data, const INTM index) const {
   data.resize(_m);
   data.setZeros();
...
template <typename T>
void SpSPAMS_Matrix<T>::setData(T* v, INTM* r, INTM* pB, INTM* pE, INTM m, INTM n, INTM nzmax) {
   this->clear();
   _externAlloc =true;
    _v = v;
    _r=r;
...
template<typename T>
void SpSPAMS_Matrix<T>::getGroup(SPAMS_Matrix<T>& data, const vector_groups& groups,  const INTM i) const {
   const group& gr = groups[i];
   const INTM N = gr.size();
   data.resize(_m,N);
   INTM count=0;
...
/// compute the sum of the matrix elements
template <typename T> inline T SpSPAMS_Matrix<T>::asum() const {
   return cblas_asum<T>(_pB[_n],_v,1);
};

/// compute the sum of the matrix elements
template <typename T> inline T SpSPAMS_Matrix<T>::normFsq() const {
   return cblas_dot<T>(_pB[_n],_v,1,_v,1);
};

template <typename T>
inline void SpSPAMS_Matrix<T>::add_direct(const SpSPAMS_Matrix<T>& mat, const T a) {
...
template <typename T>
inline void SpSPAMS_Matrix<T>::copy_direct(const SpSPAMS_Matrix<T>& mat) {
...
template <typename T>
inline T SpSPAMS_Matrix<T>::dot_direct(const SpSPAMS_Matrix<T>& mat) const {
   Vector<T> v2(mat._v,_pB[_n]);
   Vector<T> v1(_v,_pB[_n]);
   return v1.dot(v2);
}
...
/// clear the matrix
template <typename T> inline void SpSPAMS_Matrix<T>::clear() {
   if (!_externAlloc) {
      delete[](_r);
      delete[](_v);
      delete[](_pB);
...
/// resize the matrix
template <typename T> inline void SpSPAMS_Matrix<T>::resize(const INTM m, 
...
/// resize the matrix
template <typename T> inline void SpSPAMS_Matrix<T>::scal(const T a) const {
   cblas_scal<T>(_pB[_n],a,_v,1);
};

/// y <- A'*x
template <typename T>
inline void SpSPAMS_Matrix<T>::multTrans(const Vector<T>& x, Vector<T>& y,
...
/// perform b = alpha*A*x + beta*b, when x is sparse
template <typename T>
inline void SpSPAMS_Matrix<T>::multTrans(const SpVector<T>& x, Vector<T>& y, 
      const T alpha, const T beta) const {
   y.resize(_n);
   if (beta) {
      y.scal(beta);
...
/// y <- A*x
template <typename T>
inline void SpSPAMS_Matrix<T>::mult(const Vector<T>& x, Vector<T>& y,
      const T alpha, const T beta) const {
   y.resize(_m);
   if (beta) {
      y.scal(beta);
...
/// perform b = alpha*A*x + beta*b, when x is sparse
template <typename T>
inline void SpSPAMS_Matrix<T>::mult(const SpVector<T>& x, Vector<T>& y, 
      const T alpha, const T beta) const {
   y.resize(_m);
   if (beta) {
      y.scal(beta);
...
/// perform C = a*A*B + b*C, possibly transposing A or B.
template <typename T>
inline void SpSPAMS_Matrix<T>::mult(const SPAMS_Matrix<T>& B, SPAMS_Matrix<T>& C, 
      const bool transA, const bool transB,
      const T a, const T b) const {
   if (transA) {
      if (transB) {
...
/// perform C = a*A*B + b*C, possibly transposing A or B.
template <typename T>
inline void SpSPAMS_Matrix<T>::mult(const SpSPAMS_Matrix<T>& B, SPAMS_Matrix<T>& C, 
      const bool transA, const bool transB,
      const T a, const T b) const {
   if (transA) {
      if (transB) {
...
/// perform C = a*B*A + b*C, possibly transposing A or B.
template <typename T>
inline void SpSPAMS_Matrix<T>::multSwitch(const SPAMS_Matrix<T>& B, SPAMS_Matrix<T>& C, 
      const bool transA, const bool transB,
      const T a, const T b) const {
   B.mult(*this,C,transB,transA,a,b);
};
...
template <typename T>
inline T SpSPAMS_Matrix<T>::dot(const SPAMS_Matrix<T>& x) const {
   T sum=0;
   for (INTM i = 0; i<_n; ++i)
      for (INTM j = _pB[i]; j<_pE[i]; ++j) {
         sum+=_v[j]*x(_r[j],j);
...
template <typename T>
inline void SpSPAMS_Matrix<T>::copyRow(const INTM ind, Vector<T>& x) const {
   x.resize(_n);
   x.setZeros();
   for (INTM i = 0; i<_n; ++i) {
      for (INTM j = _pB[i]; j<_pE[i]; ++j) {
...
template <typename T> 
inline void SpSPAMS_Matrix<T>::addVecToCols(
      const Vector<T>& vec, const T a) {
...
template <typename T> 
inline void SpSPAMS_Matrix<T>::addVecToColsWeighted(
...
template <typename T> 
inline void SpSPAMS_Matrix<T>::sum_cols(Vector<T>& sum) const {
   sum.resize(_m);
   sum.setZeros();
   SpVector<T> tmp;
   for (INTM i = 0; i<_n; ++i) {
...
/// aat <- A*A'
template <typename T> inline void SpSPAMS_Matrix<T>::AAt(SPAMS_Matrix<T>& aat) const {
   INTM i,j,k;
   INTM K=_m;
   INTM M=_n;
...
template <typename T>
inline void SpSPAMS_Matrix<T>::XtX(SPAMS_Matrix<T>& XtX) const {
   XtX.resize(_n,_n);
   XtX.setZeros();
   SpVector<T> col;
   Vector<T> col_out;
...
/// aat <- A(:,indices)*A(:,indices)'
template <typename T> inline void SpSPAMS_Matrix<T>::AAt(SPAMS_Matrix<T>& aat,
      const Vector<INTM>& indices) const {
   INTM i,j,k;
   INTM K=_m;
   INTM M=indices.n();
...
/// aat <- sum_i w_i A(:,i)*A(:,i)'
template <typename T> inline void SpSPAMS_Matrix<T>::wAAt(const Vector<T>& w,
      SPAMS_Matrix<T>& aat) const {
   INTM i,j,k;
   INTM K=_m;
   INTM M=_n;
...
/// XAt <- X*A'
template <typename T> inline void SpSPAMS_Matrix<T>::XAt(const SPAMS_Matrix<T>& X,
      SPAMS_Matrix<T>& XAt) const {
   INTM j,i;
   INTM n=X._m;
   INTM K=_m;
   INTM M=_n;
...
/// XAt <- X(:,indices)*A(:,indices)'
template <typename T> inline void SpSPAMS_Matrix<T>::XAt(const SPAMS_Matrix<T>& X,
      SPAMS_Matrix<T>& XAt, const Vector<INTM>& indices) const {
   INTM j,i;
   INTM n=X._m;
   INTM K=_m;
   INTM M=indices.n();
...
/// XAt <- sum_i w_i X(:,i)*A(:,i)'
template <typename T> inline void SpSPAMS_Matrix<T>::wXAt(const Vector<T>& w,
      const SPAMS_Matrix<T>& X, SPAMS_Matrix<T>& XAt, const int numThreads) const {
   INTM j,l,i;
   INTM n=X._m;
   INTM K=_m;
   INTM M=_n;
...
/// copy the sparse matrix into a dense matrix
template<typename T> inline void SpSPAMS_Matrix<T>::toFull(SPAMS_Matrix<T>& matrix) const {
   matrix.resize(_m,_n);
   matrix.setZeros();
   T* out = matrix._X;
   for (INTM i=0; i<_n; ++i) {
...
template <typename T> inline void SpSPAMS_Matrix<T>::toFullTrans(
      SPAMS_Matrix<T>& matrix) const {
   matrix.resize(_n,_m);
   matrix.setZeros();
   T* out = matrix._X;
   for (INTM i=0; i<_n; ++i) {
...
/// use the data from v, r for _v, _r
template <typename T> inline void SpSPAMS_Matrix<T>::convert(const SPAMS_Matrix<T>&vM, 
      const SPAMS_Matrix<INTM>& rM, const INTM K) {
   const INTM M = rM.n();
   const INTM L = rM.m();
   const INTM* r = rM.X();
   const T* v = vM.X();
...
/// use the data from v, r for _v, _r
template <typename T> inline void SpSPAMS_Matrix<T>::convert2(
      const SPAMS_Matrix<T>&vM, const Vector<INTM>& rv, const INTM K) {
   const INTM M = vM.n();
   const INTM L = vM.m();
   INTM* r = rv.rawX();
   const T* v = vM.X();
...
/// returns the l2 norms ^2 of the columns
template <typename T>
inline void SpSPAMS_Matrix<T>::norm_2sq_cols(Vector<T>& norms) const {
   norms.resize(_n);
   SpVector<T> col;
   for (INTM i = 0; i<_n; ++i) {
      this->refCol(i,col);
...
template <typename T>
inline void SpSPAMS_Matrix<T>::norm_0_cols(Vector<T>& norms) const {
   norms.resize(_n);
   SpVector<T> col;
   for (INTM i = 0; i<_n; ++i) {
      this->refCol(i,col);
...
template <typename T>
inline void SpSPAMS_Matrix<T>::norm_1_cols(Vector<T>& norms) const {
   norms.resize(_n);
   SpVector<T> col;
   for (INTM i = 0; i<_n; ++i) {
      this->refCol(i,col);
...
template <typename T> void inline SpVector<T>::toSpSPAMS_Matrix(
      SpSPAMS_Matrix<T>& out, const INTM m, const INTM n) const {
   out.resize(m,n,_L);
...
/* ****************************
 * Implementaton of ProdSPAMS_Matrix
 * ****************************/

template <typename T> ProdSPAMS_Matrix<T>::ProdSPAMS_Matrix()  {
   _DtX= NULL; 
   _X=NULL; 
   _D=NULL; 
   _high_memory=true;
...
/// Constructor. SPAMS_Matrix D'*X is represented
template <typename T> ProdSPAMS_Matrix<T>::ProdSPAMS_Matrix(const SPAMS_Matrix<T>& D,
      const bool high_memory) {
   if (high_memory) _DtX = new SPAMS_Matrix<T>();
   this->setMatrices(D,high_memory);
};

/// Constructor. SPAMS_Matrix D'*X is represented
template <typename T> ProdSPAMS_Matrix<T>::ProdSPAMS_Matrix(const SPAMS_Matrix<T>& D, const SPAMS_Matrix<T>& X,
      const bool high_memory) {
   if (high_memory) _DtX = new SPAMS_Matrix<T>();
   this->setMatrices(D,X,high_memory);
};

template <typename T> inline void ProdSPAMS_Matrix<T>::setMatrices(const SPAMS_Matrix<T>& D, const SPAMS_Matrix<T>& X,
      const bool high_memory)  {
...
template <typename T> inline void ProdSPAMS_Matrix<T>::setMatrices(
      const SPAMS_Matrix<T>& D, const bool high_memory) {
   _high_memory=high_memory;
   _m = D.n(); 
   _n = D.n();
   if (high_memory) {
...
/// compute DtX(:,i)
template <typename T> inline void ProdSPAMS_Matrix<T>::copyCol(const INTM i, Vector<T>& DtXi) const {
   if (_high_memory) {
      _DtX->copyCol(i,DtXi);
   } else {
      Vector<T> Xi;
...
/// compute DtX(:,i)
template <typename T> inline void ProdSPAMS_Matrix<T>::extract_rawCol(const INTM i,T* DtXi) const {
   if (_high_memory) {
...
template <typename T> inline void ProdSPAMS_Matrix<T>::add_rawCol(const INTM i,T* DtXi,
      const T a) const {
   if (_high_memory) {
...
template <typename T> void inline ProdSPAMS_Matrix<T>::addDiag(const T diag) {
   if (_m == _n) {
      if (_high_memory) {
         _DtX->addDiag(diag);
      } else {
...
template <typename T> inline T ProdSPAMS_Matrix<T>::operator[](const INTM index) const {
   if (_high_memory) {
      return (*_DtX)[index];
   } else {
...
template <typename T> inline T ProdSPAMS_Matrix<T>::operator()(const INTM index1,
...
template <typename T> void inline ProdSPAMS_Matrix<T>::diag(Vector<T>& diag) const {
   if (_m == _n) {
      if (_high_memory) {
         _DtX->diag(diag);
      } else {
...
template <typename T> class SubSPAMS_Matrix : public AbstractSPAMS_Matrix<T> {

   public:
      SubSPAMS_Matrix(AbstractSPAMS_Matrix<T>& G, Vector<INTM>& indI, Vector<INTM>& indJ);
   ...
   private:
      ...
      AbstractSPAMS_Matrix<T>* _matrix;
};

template <typename T> 
SubSPAMS_Matrix<T>::SubSPAMS_Matrix(AbstractSPAMS_Matrix<T>& G, Vector<INTM>& indI, Vector<INTM>& indJ) {
   _matrix = &G;
   _indicesI.copy(indI);
   _indicesJ.copy(indJ);
};

template <typename T> void inline SubSPAMS_Matrix<T>::convertIndicesI(
...
template <typename T> void inline SubSPAMS_Matrix<T>::convertIndicesJ(
      Vector<INTM>& ind) const {
   INTM* pr_ind = ind.rawX();
   for (INTM i = 0; i<ind.n(); ++i) {
      if (pr_ind[i] == -1) break;
...
template <typename T> void inline SubSPAMS_Matrix<T>::extract_rawCol(const INTM i, T* pr) const {
...
template <typename T> inline void SubSPAMS_Matrix<T>::copyCol(const INTM i, 
      Vector<T>& DtXi) const {
   this->extract_rawCol(i,DtXi.rawX());
};

template <typename T> void inline SubSPAMS_Matrix<T>::add_rawCol(const INTM i, T* pr,
      const T a) const {
   INTM* pr_ind=_indicesI.rawX();
   INTM* pr_ind2=_indicesJ.rawX();
   for (INTM j = 0; j<_indicesI.n(); ++j) {
...
template <typename T> void inline SubSPAMS_Matrix<T>::diag(Vector<T>& diag) const {
   T* pr = diag.rawX();
   INTM* pr_ind=_indicesI.rawX();
   for (INTM j = 0; j<_indicesI.n(); ++j) {
...
template <typename T> inline T SubSPAMS_Matrix<T>::operator()(const INTM index1, 
      const INTM index2) const {
   return (*_matrix)(_indicesI[index1],_indicesJ[index2]);
}

/// SPAMS_Matrix with shifts
template <typename T> class ShiftSPAMS_Matrix : public AbstractSPAMS_MatrixB<T> {
   public:
      ShiftSPAMS_Matrix(const AbstractSPAMS_MatrixB<T>& inputmatrix, const int shifts, const bool center = false) : _shifts(shifts), _inputmatrix(&inputmatrix), _centered(false) {
      ...
      /// XtX = A'*A
      virtual void XtX(SPAMS_Matrix<T>& XtX) const;

      virtual void copyRow(const INTM i, Vector<T>& x) const;

      virtual void copyTo(SPAMS_Matrix<T>& copy) const;
      virtual T dot(const SPAMS_Matrix<T>& x) const;

      virtual void print(const string& name) const;

      virtual ~ShiftSPAMS_Matrix() {  };
   ...
   private:
      ...
      int _shifts;
      Vector<T> _means;
      const AbstractSPAMS_MatrixB<T>* _inputmatrix;
      bool _centered;
};

template <typename T> void ShiftSPAMS_Matrix<T>::multTrans(const
...
/// perform b = alpha*A*x + beta*b, when x is sparse
template <typename T> void ShiftSPAMS_Matrix<T>::mult(const
...
/// perform C = a*A*B + b*C, possibly transposing A or B.
template <typename T> void ShiftSPAMS_Matrix<T>::mult(const SPAMS_Matrix<T>&
      B, SPAMS_Matrix<T>& C, const bool transA, const bool transB, const T a, const T
      b) const {
   cerr << "Shift SPAMS_Matrix is used in inadequate setting" << endl;
}

template <typename T> void ShiftSPAMS_Matrix<T>::mult(const SpSPAMS_Matrix<T>& B, SPAMS_Matrix<T>& C, 
      const bool transA, const bool transB, const T a, const T b) const {
   cerr << "Shift SPAMS_Matrix is used in inadequate setting" << endl;
}

/// perform C = a*B*A + b*C, possibly transposing A or B.
template <typename T> void ShiftSPAMS_Matrix<T>::multSwitch(const
      SPAMS_Matrix<T>& B, SPAMS_Matrix<T>& C, const bool transA, const bool transB,
      const T a, const T b) const {
   cerr << "Shift SPAMS_Matrix is used in inadequate setting" << endl;
}

template <typename T> void ShiftSPAMS_Matrix<T>::XtX(SPAMS_Matrix<T>& XtX) const {
   cerr << "Shift SPAMS_Matrix is used in inadequate setting" << endl;
};

template <typename T> void ShiftSPAMS_Matrix<T>::copyRow(const INTM ind, Vector<T>& x) const {
   Vector<T> sub_vec;
   const INTM mm=_inputmatrix->m();
   for (int i = 0; i<_shifts; ++i) {
...
template <typename T> void ShiftSPAMS_Matrix<T>::copyTo(SPAMS_Matrix<T>& x) const {
   cerr << "Shift SPAMS_Matrix is used in inadequate setting" << endl;
};


template <typename T> T ShiftSPAMS_Matrix<T>::dot(const SPAMS_Matrix<T>& x) const {
   cerr << "Shift SPAMS_Matrix is used in inadequate setting" << endl;
   return 0;
};

template <typename T> void ShiftSPAMS_Matrix<T>::print(const string& name) const {
   cerr << name << endl;
   cerr << "Shift SPAMS_Matrix: " << _shifts << " shifts" << endl;
   _inputmatrix->print(name);
};

/// SPAMS_Matrix with shifts
template <typename T> class DoubleRowSPAMS_Matrix : public AbstractSPAMS_MatrixB<T> {
   public:
      DoubleRowSPAMS_Matrix(const AbstractSPAMS_MatrixB<T>& inputmatrix) : _inputmatrix(&inputmatrix) { 
         _n=inputmatrix.n();
         _m=2*inputmatrix.m();
      };
      INTM n() const { return _n; };
...
template <typename T> void DoubleRowSPAMS_Matrix<T>::multTrans(const
      Vector<T>& x, Vector<T>& b, const T alpha, const T beta) const {
   const INTM mm = _inputmatrix->m();
   Vector<T> tmp(mm);
   for (INTM i = 0; i<mm; ++i) 
...
/// perform b = alpha*A*x + beta*b, when x is sparse
template <typename T> void DoubleRowSPAMS_Matrix<T>::mult(const
      SpVector<T>& x, Vector<T>& b, const T alpha, const T beta) const {
   b.resize(_m);
   if (beta==0) {
      b.setZeros();
...
/// perform C = a*A*B + b*C, possibly transposing A or B.
template <typename T> void DoubleRowSPAMS_Matrix<T>::mult(const SPAMS_Matrix<T>&
      B, SPAMS_Matrix<T>& C, const bool transA, const bool transB, const T a, const T
      b) const {
   FLAG(5)
   cerr << "Double SPAMS_Matrix is used in inadequate setting" << endl;
}

template <typename T> void DoubleRowSPAMS_Matrix<T>::mult(const SpSPAMS_Matrix<T>& B, SPAMS_Matrix<T>& C, 
      const bool transA, const bool transB, const T a, const T b) const {
   FLAG(4)
   cerr << "Double SPAMS_Matrix is used in inadequate setting" << endl;
}
...
/// perform C = a*B*A + b*C, possibly transposing A or B.
template <typename T> void DoubleRowSPAMS_Matrix<T>::multSwitch(const
      SPAMS_Matrix<T>& B, SPAMS_Matrix<T>& C, const bool transA, const bool transB,
      const T a, const T b) const {
   FLAG(3)
   cerr << "Double SPAMS_Matrix is used in inadequate setting" << endl;
...
template <typename T> void DoubleRowSPAMS_Matrix<T>::XtX(SPAMS_Matrix<T>& XtX) const {
   FLAG(2)
   cerr << "Double SPAMS_Matrix is used in inadequate setting" << endl;
};

template <typename T> void DoubleRowSPAMS_Matrix<T>::copyRow(const INTM ind, Vector<T>& x) const {
   const INTM indd2= static_cast<INTM>(floor(static_cast<double>(ind)/2.0));
   _inputmatrix->copyRow(indd2,x);
};

template <typename T> void DoubleRowSPAMS_Matrix<T>::copyTo(SPAMS_Matrix<T>& x) const {
   FLAG(1)
   cerr << "Double SPAMS_Matrix is used in inadequate setting" << endl;
};


template <typename T> T DoubleRowSPAMS_Matrix<T>::dot(const SPAMS_Matrix<T>& x) const {
   FLAG(0)
   cerr << "Double SPAMS_Matrix is used in inadequate setting" << endl;
   return 0;
};

template <typename T> void DoubleRowSPAMS_Matrix<T>::print(const string& name) const {
   cerr << name << endl;
   cerr << "Double Row SPAMS_Matrix" << endl;
   _inputmatrix->print(name);
};

// solve (A'A+lambda I)x = A'b+delta
template <typename T> void AbstractSPAMS_MatrixB<T>::ridgeCG(const Vector<T>& b, const Vector<T>& delta,
...
template <typename T> void AbstractSPAMS_MatrixB<T>::ridgeCG(const Vector<T>& b,
      Vector<T>& x, const T lambda, const T tol, const int itermax) const {
   const int m = this->m();
   const int n = this->n();
   Vector<T> copyb;
...
template <typename T> void AbstractSPAMS_MatrixB<T>::ridgeCG(const SPAMS_Matrix<T>& mb,
      SPAMS_Matrix<T>& mx, const T lambda, const T tol, const int itermax,const int numThreads) const {
   init_omp(numThreads);
...
template <typename T> void AbstractSPAMS_MatrixB<T>::ridgeCG(const SPAMS_Matrix<T>& mb, const SPAMS_Matrix<T>& mdelta,
      SPAMS_Matrix<T>& mx, const T lambda, const T tol, const int itermax,const int numThreads) const {
   init_omp(numThreads);
   const int num_probs=mb.n();
   int i = 0;
