...
template <typename T>
void gpFISTAFor(const SPAMS_Matrix<T>& A, const Vector<T>& b, Vector<T>& xCurr, const T L0 = 1.0, const T eta = 1.0/0.7, const int I = 50, const bool warm = false) {
   const int p = A.n();
   Vector<T> xPrev(p);
   if(warm)
      xPrev.copy(xCurr);
...
template <typename T>
void activeSet(const SPAMS_Matrix<T>& M, const Vector<T>& b, Vector<T>& xCurr, const T lambda2 = T(1e-5), const T epsilon = T(1e-5), bool warm = false) {
   const int m = M.m();
   const int p = M.n();
   ...
   Vector<T> xRed(L);
   Vector<T> cRed(L);
   SPAMS_Matrix<T> MRed(m,L);
   T* const pr_MRed = MRed.rawX();
   SPAMS_Matrix<T> GRed(L,L);
   T* const pr_GRed = GRed.rawX();
   SPAMS_Matrix<T> GRedinv(L, L);
...
template <typename T>
void activeSetS(const SPAMS_Matrix<T>& M, const Vector<T>& b, Vector<T>& xCurr, const SPAMS_Matrix<T>& G, const T lambda2 = 1e-5, const T epsilon = 1e-5, bool warm = false) {
   const int m = M.m();
   const int p = M.n();
   const int L = MIN(m,p)+1;
   T lam2sq =lambda2*lambda2;
