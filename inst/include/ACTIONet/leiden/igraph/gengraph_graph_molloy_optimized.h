...
private:
  ...
  // Depth-Exploration. Returns number of steps done. Stops when encounter
  // vertex of degree > dmax.
  void depth_isolated(int v, long &calls, int &left_to_explore, int dmax,
                      int *&Kbuff, bool *visited);
  // breadth-first search. Store the distance (modulo 3)  in dist[]. Returns
  // eplorated component size.
  int width_search(unsigned char *dist, int *buff, int v0 = 0,
                   int toclear = -1);
  // depth-first search.
  int depth_search(bool *visited, int *buff, int v0 = 0);
  // breadth-first search that count the number of shortest paths going from src
  // to each vertex
  int breadth_path_search(int src, int *buff, double *paths,
                          unsigned char *dist);
  // Used by traceroute_sample() ONLY
  void add_traceroute_edge(int, int, int *, double **red = NULL,
                           double t = 1.0);
  // Used by traceroute() and betweenness(). if newdeg[]=NULL, do not discover
  // edges. breadth_path_search() must have been called to give the
  // corresponding buff[],dist[],paths[] and nb_vertices
  void explore_usp(double *target, int nb_vertices, int *buff, double *paths,
                   unsigned char *dist, int *newdeg = NULL,
                   double **edge_redudancy = NULL);
  void explore_asp(double *target, int nb_vertices, int *buff, double *paths,
                   unsigned char *dist, int *newdeg = NULL,
                   double **edge_redudancy = NULL);
  void explore_rsp(double *target, int nb_vertices, int *buff, double *paths,
                   unsigned char *dist, int *newdeg = NULL,
                   double **edge_redudancy = NULL);
  ...
  int *pick_random_vertices(int &k, int *output = NULL, int nb_v = -1,
                            int *among = NULL);
...
public:
  ...
  inline int compare(const int v, const int w) {
    return deg[v] == deg[w] ? lex_comp(neigh[v], neigh[w], deg[v])
                            : (deg[v] > deg[w] ? -1 : 1);
  };
  ...
  // "Optimized-Fab" Shuffle (Optimized heuristic of Gkantsidis algo, with
  // isolated pairs)
  ...
  double traceroute_sample(int mode, int nb_src, int *src, int nb_dst, int *dst,
                           double *redudancy = NULL,
                           double **edge_redudancy = NULL);
  // does one breadth-first search and returns the average_distance.
  double avg_dist(unsigned char *dist, int *buff, int v0, int &nb_vertices,
                  int toclear = -1);
  ...
  // Path sampling. Input is nb_dst[] and dst[]. nb_dst[v],dst[v] describe all
  // paths (v,x)
  double path_sampling(int *nb_dst, int *dst = NULL, double *redudancies = NULL,
                       double **edge_redudancy = NULL);
  // keep only core (tree parts are deleted). Returns number of removed
  // vertices.
  int core();
  ...
  void sort();
  // sort the vertices according to their degrees (highest first) and to their
  // adjacency lists (lexicographic)
  ...
  // pick k random vertices of degree > 0. If k \in [0,1[, k is understood as a
  // density.
  int *pick_random_src(double k, int *nb = NULL, int *buff = NULL,
                       int nb_v = -1, int *among = NULL);
  // pick k random vertices of degree > 0. If k \in [0,1], k is understood as a
  // density.
  int *pick_random_dst(double k, int *nb = NULL, int *buff = NULL,
                       int nb_v = -1, int *among = NULL);
...
#define VERIFY_NOARCS 2
  ...
  public:
    ...
    // Try to shuffle n times. Return true if at the end, the graph was still
  connected. bool try_shuffle(int T, int K);
