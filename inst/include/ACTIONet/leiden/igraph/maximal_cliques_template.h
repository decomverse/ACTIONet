...
#define RESTYPE                                                                \
  igraph_vector_int_t *subset, igraph_vector_ptr_t *res, igraph_integer_t *no, \
      FILE *outfile
...
#define RECORD                                                                 \
  do {                                                                         \
    ...
    if (outfile) {                                                             \
      igraph_vector_int_fprint(R, outfile);                                    \
    }                                                                          \
  } while (0)
#define FINALLY                                                                \
  do {                                                                         \
...
#define RESTYPE igraph_clique_handler_t *cliquehandler_fn, void *arg
...
int FUNCTION(igraph_i_maximal_cliques_bk,
             SUFFIX)(igraph_vector_int_t *PX, int PS, int PE, int XS, int XE,
                     int oldPS, int oldXE, igraph_vector_int_t *R,
                     igraph_vector_int_t *pos, igraph_adjlist_t *adjlist,
                     RESTYPE, igraph_vector_int_t *nextv,
                     igraph_vector_int_t *H, int min_size, int max_size) {
  ...
  if (PS > PE && XS > XE) {
    ...
    int pivot, mynextv;
    igraph_i_maximal_cliques_select_pivot(PX, PS, PE, XS, XE, pos, adjlist,
                                          &pivot, nextv, oldPS, oldXE);
    while ((mynextv = igraph_vector_int_pop_back(nextv)) != -1) {
      int newPS, newXE;

      /* Going down, prepare */
      igraph_i_maximal_cliques_down(PX, PS, PE, XS, XE, pos, adjlist, mynextv,
                                    R, &newPS, &newXE);
      /* Recursive call */
      err = FUNCTION(igraph_i_maximal_cliques_bk,
                     SUFFIX)(PX, newPS, PE, XS, newXE, PS, XE, R, pos, adjlist,
                             RESNAME, nextv, H, min_size, max_size);
      ...
      if (igraph_vector_int_tail(nextv) != -1) {
        igraph_i_maximal_cliques_PX(PX, PS, &PE, &XS, XE, pos, adjlist, mynextv,
                                    H);
      }
    }
  }
...
int FUNCTION(igraph_maximal_cliques, SUFFIX)(const igraph_t *graph, RESTYPE,
  ...
  err = FUNCTION(igraph_i_maximal_cliques_bk,
                 SUFFIX)(&PX, PS, PE, XS, XE, PS, XE, &R, &pos, &adjlist,
                         RESNAME, &nextv, &H, min_size, max_size);
  if (err == IGRAPH_STOP) {
    break;
  } else {
