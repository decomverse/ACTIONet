...
public:
  ...
  // simple power-law constructor : Pk = int((x+k0)^(-exp),x=k..k+1), with k0 so
  // that avg(X)=z
  degree_sequence(int n, double exp, int degmin, int degmax,
  ...
  // unbind the deg[] vector (so that it doesn't get deleted when the class is
  // destroyed)
  void detach();
