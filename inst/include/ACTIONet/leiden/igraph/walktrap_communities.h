...
public:
  ...
  int *vertices; // the vertices corresponding to the stored probabilities, 0 if
                 // all the probabilities are stored
  float *P;      // the probabilities

  long memory(); // the memory (in Bytes) used by the object
  double compute_distance(
      const Probabilities *P2) const; // compute the squared distance r^2
                                      // between this probability vector and P2
  Probabilities(int community); // compute the probability vector of a community
  Probabilities(int community1,
                int community2); // merge the probability vectors of two
                                 // communities in a new one
...
public:
  ...
  float total_weight; // sum of the weight of all the edges of the community (an
                      // edge between two communities is a half-edge for each
                      // community)

  int sub_communities[2]; // the two sub sommunities, -1 if no sub communities;
  int sub_community_of;   // number of the community in which this community has
                          // been merged
  // 0 if the community is active
  // -1 if the community is not used

  void merge(Community &C1,
             Community &C2); // create a new community by merging C1 an C2
  void add_neighbor(Neighbor *N);
  void remove_neighbor(Neighbor *N);
  float min_delta_sigma(); // compute the minimal delta sigma among all the
...
public:
  ...
  Min_delta_sigma_heap
      *min_delta_sigma; // the min delta_sigma of the community with a saved
                        // probability vector (for memory management)
  ...
  Communities(Graph *G, int random_walks_length = 3, long max_memory = -1,
              igraph_matrix_t *merges = 0,
              igraph_vector_t *modularity = 0); // Constructor
  ~Communities();                               // Destructor

  void merge_communities(
      Neighbor *N); // create a community by merging two existing communities
