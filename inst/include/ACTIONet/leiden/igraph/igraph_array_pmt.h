...
#define IGRAPH_ARRAY3_INIT_FINALLY(a, n1, n2, n3)                              \
  do {                                                                         \
    IGRAPH_CHECK(igraph_array3_init(a, n1, n2, n3));                           \
    IGRAPH_FINALLY(igraph_array3_destroy, a);                                  \
  } while (0)
#endif

#ifndef ARRAY3
#define ARRAY3(m, i, j, k)                                                     \
  ((m).data.stor_begin[(m).n1n2 * (k) + (m).n1 * (j) + (i)])
#endif

int FUNCTION(igraph_array3, init)(TYPE(igraph_array3) * a, long int n1,
...
long int FUNCTION(igraph_array3, n)(const TYPE(igraph_array3) * a,
                                    long int idx);
int FUNCTION(igraph_array3, resize)(TYPE(igraph_array3) * a, long int n1,
                                    long int n2, long int n3);
