...
template <class L_DATA> class DLItem {
...
template <class L_DATA> class DL_Indexed_List : virtual public DLList<L_DATA> {
  friend class DLList_Iter<L_DATA>;
...
public:
  ...
  bool Swap(DLList_Iter<L_DATA>); // swapt die beiden Elemente, wenn sie in der
                                  // gleichen Liste stehen!!
};
...
HUGE_INDEX HugeArray<DATA>::get_huge_index(unsigned long index) {
  ...
  // wie oft muessen wir help_index nach links shiften, damit das 31. Bit
  // gesetzt ist??
  ...
  help_index = 1 << h_index.field_index;  // in help_index wird das hoechste
                                          // besetzte Bit von Index gesetzt
  h_index.in_field_index =
      (index ^ help_index); // index XOR help_index, womit alle bits unter dem
                            // hoechsten erhalten bleiben
  return h_index;
}

template <class DATA> DATA &HugeArray<DATA>::Set(unsigned long int index) {
  HUGE_INDEX h_index;
...
template <class DATA> DATA HugeArray<DATA>::Get(unsigned long index) {
  return (Set(index));
}

template <class DATA> DATA &HugeArray<DATA>::operator[](unsigned long index) {
  return (Set(index));
}
...
DLItem<L_DATA>::DLItem(L_DATA i, unsigned long ind)
    : item(i), index(ind), previous(0), next(0) {}

template <class L_DATA>
DLItem<L_DATA>::DLItem(L_DATA i, unsigned long ind, DLItem<L_DATA> *p,
                       DLItem<L_DATA> *n)
    : item(i), index(ind), previous(p), next(n) {}

template <class L_DATA> DLItem<L_DATA>::~DLItem() {
  // delete item;      //eigentlich muessten wir pruefen, ob item ueberhaupt ein
  // Pointer ist... previous=NULL; next=NULL;
}

//######################################################################################################################
template <class L_DATA> DLList<L_DATA>::DLList(void) {
  head = tail = NULL;
  number_of_items = 0;
  head = new DLItem<L_DATA>(
      NULL,
      0); // fuer head und Tail gibt es das gleiche Array-Element!! Vorsicht!!
...
template <class L_DATA> DLList<L_DATA>::~DLList() {
...
DLItem<L_DATA> *DLList<L_DATA>::pInsert(L_DATA data, DLItem<L_DATA> *pos) {
  DLItem<L_DATA> *i =
      new DLItem<L_DATA>(data, number_of_items + 1, pos->previous, pos);
  if (i) {
    pos->previous->next = i;
    pos->previous = i;
    number_of_items++;
...
// privates delete
template <class L_DATA> L_DATA DLList<L_DATA>::pDelete(DLItem<L_DATA> *i) {
  L_DATA data = i->item;
...
// oeffentliche Delete
template <class L_DATA> int DLList<L_DATA>::Delete(unsigned long pos) {
  if ((pos < 0) || (pos > (number_of_items))) {
    return (0);
  }
  DLItem<L_DATA> *cur = head;
...
// oeffentliche Delete
template <class L_DATA> int DLList<L_DATA>::fDelete(L_DATA data) {
...
template <class L_DATA> L_DATA DLList<L_DATA>::Push(L_DATA data) {
  DLItem<L_DATA> *tmp;
...
template <class L_DATA> L_DATA DLList<L_DATA>::Pop(void) {
  return (pDelete(tail->previous));
}

template <class L_DATA> L_DATA DLList<L_DATA>::Get(unsigned long pos) {
...
template <class L_DATA> int DLList<L_DATA>::Enqueue(L_DATA data) {
  return (pInsert(data, tail) != 0);
}

template <class L_DATA> L_DATA DLList<L_DATA>::Dequeue(void) {
  return (pDelete(head->next));
}

// gibt Index des gesuchte Listenelement zurueck, besser waere eigentlich zeiger
template <class L_DATA> unsigned long DLList<L_DATA>::Is_In_List(L_DATA data) {
  DLItem<L_DATA> *cur = head, *next;
  unsigned long pos = 0;
  while (cur) {
    next = cur->next;
...
template <class L_DATA> DL_Indexed_List<L_DATA>::~DL_Indexed_List() {
...
// privates Insert
template <class L_DATA>
DLItem<L_DATA> *DL_Indexed_List<L_DATA>::pInsert(L_DATA data,
                                                 DLItem<L_DATA> *pos) {
...
template <class L_DATA> L_DATA DL_Indexed_List<L_DATA>::Push(L_DATA data) {
  DLItem<L_DATA> *tmp;
...
template <class L_DATA> L_DATA DL_Indexed_List<L_DATA>::Pop(void) {
  return (pDelete(this->tail->previous));
}

template <class L_DATA> L_DATA DL_Indexed_List<L_DATA>::Get(unsigned long pos) {
...
template <class L_DATA> ClusterList<L_DATA>::~ClusterList() {
...
//#####################################################################################
template <class L_DATA> DLList_Iter<L_DATA>::DLList_Iter() {
  list = NULL;
  current = NULL;
  end_reached = true;
}

template <class L_DATA> L_DATA DLList_Iter<L_DATA>::Next(void) {
...
template <class L_DATA> L_DATA DLList_Iter<L_DATA>::Previous(void) {
...
template <class L_DATA> L_DATA DLList_Iter<L_DATA>::First(DLList<L_DATA> *l) {
  list = l;
...
template <class L_DATA> L_DATA DLList_Iter<L_DATA>::Last(DLList<L_DATA> *l) {
  list = l;
...
template <class L_DATA> bool DLList_Iter<L_DATA>::Swap(DLList_Iter<L_DATA> b) {
  L_DATA h;
  if (list != b.list) {
