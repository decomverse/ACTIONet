...
inline void qsort(int *v, int t) {
  if (t < 15) {
    ...
    int x = partitionne(v, t,
                        med3(v[t >> 1], v[(t >> 2) + 2], v[t - (t >> 1) - 2]));
    qsort(v, x);
    qsort(v + x, t - x);
  }
}
...
inline int qsort_median(int *v, int t, int pos) {
  ...
  int x =
      partitionne(v, t, med3(v[t >> 1], v[(t >> 2) + 2], v[t - (t >> 1) - 2]));
  if (pos < x) {
...
inline void qsort(int *mem, int *v, int t) {
  if (t < 15) {
    ...
    int p =
        med3(mem[v[t >> 1]], mem[v[(t >> 2) + 3]], mem[v[t - (t >> 1) - 3]]);
    int i = 0;
    int j = t - 1;
    while (i < j) {
      while (i <= j && mem[v[i]] < p) {
...
// merge two sorted arays in their intersection. Store the result in first
// array, and return length
...
inline void lex_qsort(int **l, int *v, int t, int s) {
  ...
  if (t < _CRITICAL_SIZE_QSORT) {
    ...
    int *p =
        lex_med3(l[v[t >> 1]], l[v[(t >> 2) + 2]], l[v[t - (t >> 1) - 2]], s);
    int i = 0;
    int j = t - 1;
    //    printf("pivot = %d\n",p);
    while (i < j) {
...
inline int mix_comp_indirect(int *key, int a, int b, int **neigh, int *degs) {
  if (key[a] < key[b]) {
    ...
    int cmp =
        lex_comp_indirect(key, neigh[a], neigh[b], qsort_min(degs[a], degs[b]));
    if (cmp == 0) {
      if (degs[a] > degs[b]) {
        return -1;
      }
...
inline int mix_med3_indirect(int *key, int a, int b, int c, int **neigh,
                             int *degs) {
...
inline void mix_isort_indirect(int *key, int *v, int t, int **neigh,
                               int *degs) {
  if (t < 2) {
    return;
  }
...
inline void mix_qsort_indirect(int *key, int *v, int t, int **neigh,
                               int *degs) {
  if (t < 15) {
    mix_isort_indirect(key, v, t, neigh, degs);
  } else {
    int p = mix_med3_indirect(key, v[t >> 1], v[(t >> 2) + 2],
                              v[t - (t >> 1) - 2], neigh, degs);
    int i = 0;
    int j = t - 1;
    //    printf("pivot = %d\n",p);
    while (i < j) {
