...
#define GRAPH_IS_EDGE(g, i, j)                                                 \
  (((i) < ((g)->n)) ? SET_CONTAINS((g)->edges[(i)], (j)) : FALSE)
...
UNUSED_FUNCTION INLINE static int graph_subgraph_weight(graph_t *g, set_t s) {
  int i, j;
  int count = 0;
  setelement e;
...
UNUSED_FUNCTION INLINE static int graph_vertex_degree(graph_t *g, int v) {
