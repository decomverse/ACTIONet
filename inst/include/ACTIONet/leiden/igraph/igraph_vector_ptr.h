...
#define IGRAPH_VECTOR_PTR_NULL                                                 \
  { 0, 0, 0, 0 }
#define IGRAPH_VECTOR_PTR_INIT_FINALLY(v, size)                                \
  do {                                                                         \
    IGRAPH_CHECK(igraph_vector_ptr_init(v, size));                             \
    IGRAPH_FINALLY(igraph_vector_ptr_destroy, v);                              \
  } while (0)

DECLDIR int igraph_vector_ptr_init(igraph_vector_ptr_t *v, long int size);
DECLDIR int igraph_vector_ptr_init_copy(igraph_vector_ptr_t *v, void **data,
                                        long int length);
DECLDIR const igraph_vector_ptr_t *
igraph_vector_ptr_view(const igraph_vector_ptr_t *v, void *const *data,
                       long int length);
...
DECLDIR int igraph_vector_ptr_insert(igraph_vector_ptr_t *v, long int pos,
                                     void *e);
DECLDIR void *igraph_vector_ptr_e(const igraph_vector_ptr_t *v, long int pos);
DECLDIR void igraph_vector_ptr_set(igraph_vector_ptr_t *v, long int pos,
                                   void *value);
