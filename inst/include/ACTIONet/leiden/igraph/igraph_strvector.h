...
#define IGRAPH_STRVECTOR_NULL                                                  \
  { 0, 0 }
#define IGRAPH_STRVECTOR_INIT_FINALLY(v, size)                                 \
  do {                                                                         \
    IGRAPH_CHECK(igraph_strvector_init(v, size));                              \
    IGRAPH_FINALLY((igraph_finally_func_t *)igraph_strvector_destroy, v);      \
  } while (0)
...
DECLDIR void igraph_strvector_get(const igraph_strvector_t *sv, long int idx,
                                  char **value);
...
DECLDIR void igraph_strvector_remove_section(igraph_strvector_t *v,
                                             long int from, long int to);
DECLDIR void igraph_strvector_remove(igraph_strvector_t *v, long int elem);
DECLDIR void igraph_strvector_move_interval(igraph_strvector_t *v,
...
DECLDIR void igraph_strvector_permdelete(igraph_strvector_t *v,
                                         const igraph_vector_t *index,
                                         long int nremove);
DECLDIR void igraph_strvector_remove_negidx(igraph_strvector_t *v,
