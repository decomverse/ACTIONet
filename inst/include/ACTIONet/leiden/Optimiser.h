...
public:
  ...
  double move_nodes(vector<MutableVertexPartition *> partitions,
                    vector<double> layer_weights);
  double move_nodes(vector<MutableVertexPartition *> partitions,
  ...
  double merge_nodes(vector<MutableVertexPartition *> partitions,
                     vector<double> layer_weights);
  double merge_nodes(vector<MutableVertexPartition *> partitions,
                     vector<double> layer_weights, int consider_comms);
  ...
  int consider_comms;   // Indicates how communities will be considered for
                        // improvement. Should be one of the parameters below
  int refine_partition; // Refine partition before aggregating
  int refine_consider_comms; // Indicates how communities will be considered for
                             // improvement within the refinement. Should be one
  ...
  int consider_empty_community; // Determine whether to consider moving nodes to
                                // an empty community

  static const int ALL_COMMS = 1; // Consider all communities for improvement.
  static const int ALL_NEIGH_COMMS =
      2; // Consider all neighbour communities for improvement.
  static const int RAND_COMM =
      3; // Consider a random commmunity for improvement.
